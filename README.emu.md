...
```bash
# compile
make EMULATION_DEVICE_EN=1 -j32 gitinfo && \
make EMULATION_DEVICE_EN=1 -j32 src/ckernels && \
make EMULATION_DEVICE_EN=1 -j32 src/net2pipe && \
...
ZEBU_PHYSICAL_LOCATION=U0.M0=U0.M3 LD_LIBRARY_PATH=$$ZEBU_IP_ROOT/lib:$$LD_LIBRARY_PATH ./build/test/loader/tests/test_pybuda_ram --emulation --arch grayskull --netlist /proj_soc/mlausin/emu_buda/ws_buda_1/BudaBackEnd/loader/tests/net_basic/ram.yaml --num-pushes 128 |& tee emu.log
