...
public:
    BlackholeArcMessageQueue(Cluster* cluster, const chip_id_t chip, const uint64_t base_address, const uint64_t size);

    std::array<uint32_t, BlackholeArcMessageQueue::entry_len> send_message(
        const ArcMessageType message_type, uint32_t* data, size_t num_words);

    // static std::unique_ptr<BlackholeArcMessageQueue> create_message_queue(
    //     const Cluster* cluster, const chip_id_t chip, const size_t queue_index);

private:
    void push_request(uint32_t* request);
