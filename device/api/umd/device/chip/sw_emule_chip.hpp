...
public:
    ...
    // Pool slot for a worker core, or SIZE_MAX if it has none.
...
private:
    ...
    // Tensix core → pool slot, built ONCE from the SoC descriptor rather than on first touch, so a
    // core's backing does not depend on the order the cores happen to be touched in.
