...
class SysmemBuffer {
public:
    SysmemBuffer(void* buffer_va, uint32_t buffer_size, uint64_t device_io_addr, uint64_t noc_addr);
...
private:
    ...
    // Address that is used from device point of view to access the buffer
    // over NOC request to PCIE core.
    uint64_t noc_addr;
};
