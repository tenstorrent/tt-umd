...
/**
 * The role a window plays in a map.
 *
 * FULL_TILE is a tile's config aperture: the control registers every tile exposes, as opposed to
 * the L1 a worker exposes through WORKER. A NEO tile appears in both, reached through whichever
 * window matches the address.
 *
 * COUNT doubles as the window array extent.
 */
