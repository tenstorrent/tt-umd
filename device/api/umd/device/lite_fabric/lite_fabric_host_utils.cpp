...
SystemDescriptor GetSystemDescriptor2Devices(chip_id_t mmio_device_id, chip_id_t connected_device_id) {
    ...
    for (const auto& mmio_eth_core : cluster.get_ethernet_sockets(mmio_device_id, connected_device_id)) {
        ...
        log_info(
            LogSiliconDriver, "Add tunnel from {} {} to {} {}", mmio_device_id, mmio_eth_core, other_device, other_core);
    }
...
std::pair<const ll_api::memory&, uint32_t> CompileLiteFabric(
    ...
    log_info(LogSiliconDriver, "Building LiteFabric. Reference device {} {}", device->id(), logical_core);
...
void SetPC(Chip* chip, tt_cxy_pair virtual_core, uint32_t pc_addr, uint32_t pc_val) {
    chip->write_to_device((void*)&pc_val, sizeof(uint32_t), virtual_core, pc_addr);
}
...
void WaitForState(Chip* chip, tt_cxy_pair virtual_core, uint32_t addr, lite_fabric::InitState state) {
    ...
    while (static_cast<lite_fabric::InitState>(readback[0]) != state) {
        ...
        chip->read_from_device(readback, sizeof(uint32_t), virtual_core, addr);
    }
}
...
void TerminateLiteFabric(Chip* chip, const SystemDescriptor& desc) {
    ...
    for (const auto& tunnel_1x : desc.tunnels_from_mmio) {
        ...
        chip->write_to_device((void*)&enabled, sizeof(uint32_t), tunnel_1x.mmio_cxy_virtual(), routing_enabled_address);
    }
    cluster.l1_barrier(0);
}
