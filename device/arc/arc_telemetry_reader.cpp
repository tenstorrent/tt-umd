...
void ArcTelemetryReader::initialize_telemetry(bool use_noc1) {
    auto arc_core = get_arc_core(use_noc1);
    tt_device->read_from_device(
        &entry_count, arc_core, telemetry_table_addr + sizeof(uint32_t), sizeof(uint32_t), use_noc1);
    ...
    tt_device->read_from_device(
        telemetry_tag_entries.data(), arc_core, tag_table_address, entry_count * sizeof(TelemetryTagEntry), use_noc1);

    std::vector<uint32_t> telemetry_data(entry_count);
    tt_device->read_from_device(
        telemetry_data.data(), arc_core, telemetry_values_addr, entry_count * sizeof(uint32_t), use_noc1);
    for (uint32_t i = 0; i < entry_count; ++i) {
        ...
        tt_device->read_from_device(
            &tag_offset, arc_core, telemetry_table_addr + 8 + 4 * i, sizeof(uint32_t), use_noc1);
...
uint32_t ArcTelemetryReader::read_entry(const uint8_t telemetry_tag, bool use_noc1) {
    ...
    tt_device->read_from_device(
        &telemetry_val, arc_core, telemetry_values_addr + offset * sizeof(uint32_t), sizeof(uint32_t), use_noc1);
