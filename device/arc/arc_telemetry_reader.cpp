...
void ArcTelemetryReader::initialize_telemetry() {
    ...
    tt_device->read_from_device(
        umd_use_noc1, &entry_count, arc_core, telemetry_table_addr + sizeof(uint32_t), sizeof(uint32_t));
    ...
    tt_device->read_from_device(
        umd_use_noc1, telemetry_data.data(), arc_core, telemetry_values_addr, entry_count * sizeof(uint32_t));
    ...
    for (uint32_t i = 0; i < entry_count; ++i) {
        ...
        tt_device->read_from_device(
            umd_use_noc1, &tag_offset, arc_core, telemetry_table_addr + 8 + 4 * i, sizeof(uint32_t));
...
uint32_t ArcTelemetryReader::read_entry(const uint8_t telemetry_tag) {
    ...
    tt_device->read_from_device(
        umd_use_noc1, &telemetry_val, arc_core, telemetry_values_addr + offset * sizeof(uint32_t), sizeof(uint32_t));
