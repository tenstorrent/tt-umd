// SPDX-FileCopyrightText: © 2024 Tenstorrent Inc.
//
// SPDX-License-Identifier: Apache-2.0

#include "umd/device/arc/blackhole_arc_messenger.hpp"

#include "umd/device/tt_device/tt_device.hpp"

namespace tt::umd {

BlackholeArcMessenger::BlackholeArcMessenger(TTDevice* tt_device, bool use_noc1) : ArcMessenger(tt_device) {
    blackhole_arc_msg_queue = BlackholeArcMessageQueue::get_blackhole_arc_message_queue(
        tt_device, BlackholeArcMessageQueueIndex::APPLICATION, use_noc1);
}

uint32_t BlackholeArcMessenger::send_message(
    const uint32_t msg_code,
    std::vector<uint32_t>& return_values,
    const std::vector<uint32_t>& args,
    const std::chrono::milliseconds timeout_ms,
    bool use_noc1) {
    auto lock = lock_manager.acquire_mutex(MutexType::ARC_MSG, tt_device->get_pci_device()->get_device_num());
    return blackhole_arc_msg_queue->send_message((ArcMessageType)msg_code, args, timeout_ms, use_noc1);
}

}  // namespace tt::umd
