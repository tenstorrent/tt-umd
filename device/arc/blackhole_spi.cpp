...
void BlackholeSPI::read(uint32_t addr, uint8_t* data, size_t size) {
    ...
    while (bytes_read < size) {
        ...
        tt_device_->read_block(buffer_addr, chunk_size, data + bytes_read);
        bytes_read += chunk_size;
    }
}

void BlackholeSPI::write(uint32_t addr, const uint8_t* data, size_t size) {
    if (size == 0) {
        return;
    }
    ...
    while (bytes_written < size) {
        ...
        tt_device_->write_block(buffer_addr, chunk_size, data + bytes_written);
