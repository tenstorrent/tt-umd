...
void BlackholeSPITTDevice::write(uint32_t addr, const uint8_t* data, size_t size, bool skip_write_to_spi) {
    ...
    if (need_lock_unlock) {
        ...
        uint32_t rc =
            send_arc_command(firmware, static_cast<uint32_t>(blackhole::ArcMessageType::SPI_UNLOCK), unlock_ret, {});
        if (rc != 0) {
        ...
        if (!skip_write_to_spi) {
            ...
            if (rc != 0) {
                ...
                if (need_lock_unlock) {
                    ...
                    send_arc_command(firmware, static_cast<uint32_t>(blackhole::ArcMessageType::SPI_LOCK), lock_ret, {});
                }
    ...
    if (need_lock_unlock) {
        std::vector<uint32_t> lock_ret;
        uint32_t rc = send_arc_command(firmware, static_cast<uint32_t>(blackhole::ArcMessageType::SPI_LOCK), lock_ret, {});
        if (rc != 0) {
