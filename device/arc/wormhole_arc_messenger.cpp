...
uint32_t WormholeArcMessenger::send_message(
    ...
    if (args.size() > 2) {
        UMD_THROW("Wormhole ARC messenger only supports 2 arguments, but {} were provided.", args.size());
    }
    ...
    if (args.size() >= 1) {
        if (args[0] > 0xFFFF) {
            UMD_THROW("Argument 0 is 0x{:x}, which exceeds uint16_t maximum (0xFFFF) for Wormhole.", args[0]);
        }
    ...
    if (args.size() >= 2) {
        if (args[1] > 0xFFFF) {
            UMD_THROW("Argument 1 is 0x{:x}, which exceeds uint16_t maximum (0xFFFF) for Wormhole.", args[1]);
        }
    ...
    while (true) {
        ...
        if ((duration.count() > timeout_ms.count()) && (timeout_ms != std::chrono::milliseconds(0))) {
            UMD_THROW(fmt::format("Timed out after waiting {} ms for ARC to respond.", timeout_ms));
        }
