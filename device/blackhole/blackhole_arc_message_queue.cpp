...
BlackholeArcMessageQueue::BlackholeArcMessageQueue(
    TTDevice* tt_device, const uint64_t base_address, const uint64_t size) :
    base_address(base_address), size(size), tt_device(tt_device) {}

void BlackholeArcMessageQueue::read_words(uint32_t* data, size_t num_words, size_t offset) {
    tt_device->read_from_arc(data, base_address + offset * sizeof(uint32_t), num_words * sizeof(uint32_t));
}
...
void BlackholeArcMessageQueue::write_words(uint32_t* data, size_t num_words, size_t offset) {
    tt_device->write_to_arc(data, base_address + offset * sizeof(uint32_t), num_words * sizeof(uint32_t));
}
...
std::unique_ptr<BlackholeArcMessageQueue> BlackholeArcMessageQueue::get_blackhole_arc_message_queue(
    ...
    tt_device->read_from_arc(&queue_control_block, queue_control_block_addr, sizeof(uint64_t));
    ...
    return std::make_unique<tt::umd::BlackholeArcMessageQueue>(tt_device, msg_queue_base, num_entries_per_queue);
}
}  // namespace tt::umd
