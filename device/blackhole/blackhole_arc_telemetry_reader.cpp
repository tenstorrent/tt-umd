...
void BlackholeArcTelemetryReader::initialize_telemetry() {
    tt_device->read_from_arc(&telemetry_table_addr, blackhole::SCRATCH_RAM_13, sizeof(uint32_t));

    tt_device->read_from_device(&entry_count, arc_core, telemetry_table_addr + sizeof(uint32_t), sizeof(uint32_t));

    tt_device->read_from_arc(&telemetry_values_addr, blackhole::SCRATCH_RAM_12, sizeof(uint32_t));
