...
void Chip::noc_multicast_write(void* dst, size_t size, CoreCoord core_start, CoreCoord core_end, uint64_t addr) {
    ...
    if (core_start.core_type != core_end.core_type ||
        (core_start.core_type != CoreType::TENSIX && core_start.core_type != CoreType::ETH)) {
        TT_THROW("noc_multicast_write is only supported for Tensix and Eth cores.");
    }
