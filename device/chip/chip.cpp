...
void Chip::wait_chip_to_be_ready(bool use_noc1) {
    wait_eth_cores_training(timeout::ETH_TRAINING_TIMEOUT, use_noc1);
    wait_dram_cores_training(timeout::DRAM_TRAINING_TIMEOUT, use_noc1);
}

void Chip::wait_eth_cores_training(const std::chrono::milliseconds timeout_ms, bool use_noc1) {
    ...
    for (const CoreCoord& eth_core : eth_cores) {
        ...
        if (get_tt_device()->get_arch() == tt::ARCH::WORMHOLE_B0) {
            ...
            actual_eth_core = translate_chip_coord_to_translated(eth_core, use_noc1);
        }

        timeout_left -= tt_device->wait_eth_core_training(actual_eth_core, timeout_left, use_noc1);
    }
}

void Chip::wait_dram_cores_training(const std::chrono::milliseconds timeout_ms, bool use_noc1) {
    TTDevice* tt_device = get_tt_device();
    ...
    for (int dram_channel = 0; dram_channel < chip_num_dram_channels; dram_channel++) {
        ...
        tt_device->wait_dram_channel_training(dram_channel, timeout_ms, use_noc1);
    }
}

void Chip::enable_ethernet_queue(const std::chrono::milliseconds timeout_ms, bool use_noc1) {
    TT_ASSERT(
    ...
    while (msg_success != 1) {
        ...
        if (arc_msg(0xaa58, true, {0xFFFF, 0xFFFF}, timeout::ARC_MESSAGE_TIMEOUT, &msg_success, nullptr, use_noc1) ==
            HANG_READ_VALUE) {
            break;
        }
    }
...
void Chip::send_tensix_risc_reset(CoreCoord core, const TensixSoftResetOptions& soft_resets, bool use_noc1) {
    TT_ASSERT(
    ...
    get_tt_device()->set_risc_reset_state(translate_chip_coord_to_translated(core, use_noc1), valid_val, use_noc1);
}

// TODO: Remove this API once we switch to the new one.
void Chip::send_tensix_risc_reset(const TensixSoftResetOptions& soft_resets, bool use_noc1) {
    for (const CoreCoord core : soc_descriptor_.get_cores(CoreType::TENSIX)) {
        send_tensix_risc_reset(core, soft_resets, use_noc1);
    }
}

RiscType Chip::get_risc_reset_state(CoreCoord core, bool use_noc1) {
    uint32_t soft_reset_current_state =
        get_tt_device()->get_risc_reset_state(translate_chip_coord_to_translated(core, use_noc1), use_noc1);
    return get_tt_device()->get_architecture_implementation()->get_soft_reset_risc_type(soft_reset_current_state);
}

void Chip::assert_risc_reset(CoreCoord core, const RiscType selected_riscs, bool use_noc1) {
    uint32_t soft_reset_current_state =
        get_tt_device()->get_risc_reset_state(translate_chip_coord_to_translated(core, use_noc1), use_noc1);
    ...
    log_debug(
        ...
        soft_reset_update,
        soft_reset_new);
    get_tt_device()->set_risc_reset_state(translate_chip_coord_to_translated(core, use_noc1), soft_reset_new, use_noc1);
}

void Chip::deassert_risc_reset(CoreCoord core, const RiscType selected_riscs, bool staggered_start, bool use_noc1) {
    uint32_t soft_reset_current_state =
        get_tt_device()->get_risc_reset_state(translate_chip_coord_to_translated(core, use_noc1), use_noc1);
    uint32_t soft_reset_update =
        get_tt_device()->get_architecture_implementation()->get_soft_reset_reg_value(selected_riscs);
    ...
    get_tt_device()->set_risc_reset_state(
        translate_chip_coord_to_translated(core, use_noc1), soft_reset_new_with_staggered_start, use_noc1);
}

void Chip::assert_risc_reset(const RiscType selected_riscs, bool use_noc1) {
    for (const CoreCoord core : soc_descriptor_.get_cores(CoreType::TENSIX)) {
        assert_risc_reset(core, selected_riscs, use_noc1);
    }
}

void Chip::deassert_risc_reset(const RiscType selected_riscs, bool staggered_start, bool use_noc1) {
    for (const CoreCoord core : soc_descriptor_.get_cores(CoreType::TENSIX)) {
        deassert_risc_reset(core, selected_riscs, staggered_start, use_noc1);
    }
}
...
int Chip::arc_msg(
    ...
    uint32_t exit_code =
        get_tt_device()->get_arc_messenger()->send_message(msg_code, arc_msg_return_values, args, timeout_ms, use_noc1);
...
void Chip::set_power_state(DevicePowerState state, bool use_noc1) {
    int exit_code = 0;
    if (soc_descriptor_.arch == tt::ARCH::WORMHOLE_B0) {
        uint32_t msg = get_power_state_arc_msg(state);
        exit_code = arc_msg(
            wormhole::ARC_MSG_COMMON_PREFIX | msg,
            true,
            {0, 0},
        ...
        if (state == DevicePowerState::BUSY) {
            exit_code = get_tt_device()->get_arc_messenger()->send_message(
                (uint32_t)blackhole::ArcMessageType::AICLK_GO_BUSY, {}, timeout::ARC_MESSAGE_TIMEOUT, use_noc1);
        } else {
            exit_code = get_tt_device()->get_arc_messenger()->send_message(
                (uint32_t)blackhole::ArcMessageType::AICLK_GO_LONG_IDLE, {}, timeout::ARC_MESSAGE_TIMEOUT, use_noc1);
        }
    }
    TT_ASSERT(exit_code == 0, "Failed to set power state to {} with exit code: {}", (int)state, exit_code);
    wait_for_aiclk_value(get_tt_device(), state, timeout::AICLK_TIMEOUT, use_noc1);
}

tt_xy_pair Chip::translate_chip_coord_to_translated(const CoreCoord core, bool use_noc1) const {
    ...
    // NOC1 or translated space depending on the use_noc1 flag.
    // On Wormhole Tensix can use NOC1 space if use_noc1 is set to true.
    ...
    return soc_descriptor_.translate_coord_to(core, use_noc1 ? CoordSystem::NOC1 : CoordSystem::TRANSLATED);
}

void Chip::wait_for_aiclk_value(
    TTDevice* tt_device, DevicePowerState power_state, const std::chrono::milliseconds timeout_ms, bool use_noc1) {
    auto start = std::chrono::steady_clock::now();
    uint32_t target_aiclk = 0;
    if (power_state == DevicePowerState::BUSY) {
        target_aiclk = tt_device->get_max_clock_freq(use_noc1);
    } else if (power_state == DevicePowerState::LONG_IDLE) {
        target_aiclk = tt_device->get_min_clock_freq();
    }
    uint32_t aiclk = tt_device->get_clock(use_noc1);
    while (aiclk != target_aiclk) {
        ...
        if (duration.count() > timeout_ms.count()) {
            log_warning(
                ...
                target_aiclk,
                aiclk,
                tt_device->get_asic_temperature(use_noc1));
            return;
        }
        aiclk = tt_device->get_clock(use_noc1);
    }
}

void Chip::noc_multicast_write(
