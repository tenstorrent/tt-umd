...
LocalChip::LocalChip(tt_SocDescriptor soc_descriptor, int pci_device_id) :
    ...
    tlb_manager_->set_dynamic_tlb(
        "LARGE_READ_TLB", tt_device_->get_architecture_implementation()->get_mem_large_read_tlb());
    tlb_manager_->set_dynamic_tlb_ordering("LARGE_READ_TLB", tlb_data::Relaxed);
    tlb_manager_->set_dynamic_tlb(
        "LARGE_WRITE_TLB", tt_device_->get_architecture_implementation()->get_mem_large_write_tlb());
    tlb_manager_->set_dynamic_tlb_ordering("LARGE_WRITE_TLB", tlb_data::Relaxed);
    tlb_manager_->set_dynamic_tlb("REG_TLB", tt_device_->get_architecture_implementation()->get_reg_tlb());
    tlb_manager_->set_dynamic_tlb_ordering("REG_TLB", tlb_data::Relaxed);
    tlb_manager_->set_dynamic_tlb(
        "SMALL_READ_WRITE_TLB", tt_device_->get_architecture_implementation()->get_small_read_write_tlb());
    tlb_manager_->set_dynamic_tlb_ordering("SMALL_READ_WRITE_TLB", tlb_data::Relaxed);
}
