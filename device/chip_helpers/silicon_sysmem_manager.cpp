...
SiliconSysmemManager::SiliconSysmemManager(TTDevice *tt_device, uint32_t num_host_mem_channels) {
    ...
    pcie_base_ = get_pcie_base_for_arch(tt_device_->get_arch());
    UMD_ASSERT(
...
bool SiliconSysmemManager::pin_or_map_sysmem_to_device() {
    ...
    if (pcie_interface_->is_iommu_enabled()) {
...
void SiliconSysmemManager::unpin_or_unmap_sysmem() {
    ...
    if (iommu_mapping != nullptr) {
        ...
        for (int ch = 0; ch < hugepage_mapping_per_channel.size(); ch++) {
            ...
            if (hugepage_mapping.physical_address && pcie_interface_->is_mapping_buffer_to_noc_supported()) {
                ...
                pcie_interface_->unmap_for_dma(hugepage_mapping.mapping, actual_size);
            }
...
bool SiliconSysmemManager::init_hugepages(uint32_t num_host_mem_channels) {
    ...
    uint16_t pcie_device_id = static_cast<uint16_t>(tt_device_->get_device_protocol()->get_mmio_id());
    uint32_t pcie_revision = pcie_interface_->get_pci_revision();
    ...
    auto physical_device_id = tt_device_->get_communication_device_id();
...
bool SiliconSysmemManager::pin_or_map_hugepages() {
    ...
    for (int ch = 0; ch < hugepage_mapping_per_channel.size(); ch++) {
        ...
        size_t actual_size = (tt_device_->get_arch() == tt::ARCH::WORMHOLE_B0 && ch == 3)
                                 ? HUGEPAGE_CHANNEL_3_SIZE_LIMIT
                                 : hugepage_size;
        bool map_buffer_to_noc = pcie_interface_->is_mapping_buffer_to_noc_supported();
        uint64_t physical_address;
        uint64_t noc_address;
        if (map_buffer_to_noc) {
            std::tie(noc_address, physical_address) = pcie_interface_->map_hugepage_to_noc(mapping, actual_size);
            ...
            physical_address = pcie_interface_->map_for_hugepage(mapping, actual_size);
        }
