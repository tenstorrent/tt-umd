...
int SimulationTlbAllocator::allocate_tlb_index(size_t size) {
    ...
    //
    // size == 0 is handled by the same loop: `0 > group.size` is always false for
    // size_t, so every pool is considered, smallest first.
    for (TlbPool& pool : pools_) {
        if (size > pool.group.size) {
            continue;
        }
        for (size_t i = 0; i < pool.group.count; ++i) {
            if (!pool.allocated[i]) {
                pool.allocated[i] = true;
                return static_cast<int>(pool.group.base_index + i);
            }
        }
    }
...
void SimulationTlbAllocator::deallocate_tlb_index(int tlb_index) {
    ...
    if (TlbPool* pool = find_pool_for_index(tlb_index)) {
        pool->allocated[static_cast<size_t>(tlb_index) - pool->group.base_index] = false;
    }
}
...
uint64_t SimulationTlbAllocator::get_tlb_address_from_index(int tlb_index) {
    ...
    const uint64_t slot = static_cast<uint64_t>(tlb_index) - pool->group.base_index;

    // Each BAR4 resident window occupies its own region, in TLB-index order.
    if (pool->group.in_bar4) {
        return bar4_base_ + slot * pool->group.size;
    }
    ...
    for (const TlbPool& earlier : pools_) {
        ...
        if (!earlier.group.in_bar4) {
            region_offset += earlier.group.count * earlier.group.size;
        }
    }

    return bar0_base_ + region_offset + slot * pool->group.size;
}
...
SimulationTlbAllocator::TlbPool* SimulationTlbAllocator::find_pool_for_index(int tlb_index) {
    ...
    for (TlbPool& pool : pools_) {
        if (idx >= pool.group.base_index && idx < pool.group.base_index + pool.group.count) {
            return &pool;
        }
    }
    return nullptr;
...
void SimulationTlbAllocator::initialize_architecture_config() {
    ...
    for (const TlbWindowGroup& group : get_architecture_tlbs(architecture_).window_groups) {
        TlbPool pool;
        pool.group = group;
        pool.allocated.resize(group.count, false);
