...
void TLBManager::configure_tlb(bool use_noc1, tt_xy_pair core, size_t tlb_size, uint64_t address, uint64_t ordering) {
    TT_ASSERT(
...
void TLBManager::configure_tlb_kmd(
    bool use_noc1, tt_xy_pair core, size_t tlb_size, uint64_t address, uint64_t ordering) {
    TT_ASSERT(
        ordering == tlb_data::Strict || ordering == tlb_data::Posted || ordering == tlb_data::Relaxed,
        "Invalid ordering specified in Cluster::configure_tlb");
    log_debug(
