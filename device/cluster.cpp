...
SocDescriptor Cluster::construct_soc_descriptor(
    ...
    if (soc_desc_path.empty()) {
        ...
        return SocDescriptor(arch, noc_translation_table_en, harvesting_masks, chip_board_type, asic_location);

    } else {
        SocDescriptor soc_desc =
            SocDescriptor(soc_desc_path, noc_translation_table_en, harvesting_masks, chip_board_type, asic_location);
