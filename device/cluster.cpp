...
void Cluster::broadcast_pcie_tensix_risc_reset(chip_id_t chip_id, const TensixSoftResetOptions& soft_resets) {
    ...
    // We want all rows to have a reset signal sent on GS and BH.
    int num_harvested = arch_name == tt::ARCH::WORMHOLE_B0
    ...
    auto [soft_reset_reg, _] = tt_device->set_dynamic_tlb_broadcast(
        ...
        translate_chip_coord_virtual_to_translated(
            ...
            tt_xy_pair(
                ...
                architecture_implementation->get_grid_size_y() - 1 - num_harvested)),
        TLB_DATA::Posted);
...
tt::umd::CoreCoord Cluster::translate_chip_coord(
    ...
    return get_soc_descriptor(chip_id).translate_coord_to(core_coord, coord_system);
}
