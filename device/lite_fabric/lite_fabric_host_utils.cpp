...
void SetResetState(Chip* chip, const SystemDescriptor& desc, bool assert_reset) {
    for (auto tunnel_1x : desc.tunnels_from_mmio) {
        SetResetState(chip, tunnel_1x.mmio_cxy_virtual(), assert_reset);
    }
}
...
void SetPC(Chip* chip, const SystemDescriptor& desc, uint32_t pc_addr, uint32_t pc_val) {
    for (auto tunnel_1x : desc.tunnels_from_mmio) {
        SetPC(chip, tunnel_1x.mmio_cxy_virtual(), pc_addr, pc_val);
    }
}

void WaitForState(Chip* chip, tt_cxy_pair virtual_core, uint32_t addr, InitState state) {
...
void WaitForState(Chip* chip, const SystemDescriptor& desc, uint32_t addr, InitState state) {
    for (auto tunnel_1x : desc.tunnels_from_mmio) {
        WaitForState(chip, tunnel_1x.mmio_cxy_virtual(), addr, state);
    }
}
...
void LaunchLiteFabric(
    ...
    for (const auto& tunnel_1x : desc.tunnels_from_mmio) {
        SetResetState(chip, tunnel_1x.mmio_cxy_virtual(), true);
        SetPC(chip, tunnel_1x.mmio_cxy_virtual(), k_PcResetAddress, k_FirmwareStart);
        ...
        //         chip->write_to_device(&*mem_ptr, len_words * sizeof(uint32_t), tunnel_1x.mmio_cxy_virtual(), relo_addr);
        //         log_info(
        //             LogSiliconDriver,
    ...
    for (auto tunnel_1x : desc.tunnels_from_mmio) {
        SetResetState(chip, tunnel_1x.mmio_cxy_virtual(), false);
    }
    ...
    for (auto tunnel_1x : desc.tunnels_from_mmio) {
        WaitForState(
            chip, tunnel_1x.mmio_cxy_virtual(), GetStateAddress(), InitState::READY);
        // log_info(
        //     LogSiliconDriver,
...
void TerminateLiteFabric(Chip* chip, const SystemDescriptor& desc) {
    uint32_t routing_enabled_address = MEM_LITE_FABRIC_CONFIG_BASE +
                                       offsetof(LiteFabricMemoryMap, config) +
    ...
    for (const auto& tunnel_1x : desc.tunnels_from_mmio) {
        ...
        CoreCoord umd_core = CoreCoord(tunnel_1x.mmio_core_virtual.x, tunnel_1x.mmio_core_virtual.y, CoreType::ETH, CoordSystem::TRANSLATED);
