...
void dump_timings(const char* fn, std::size_t bytes, const std::vector<OpTiming>& ops) {
    ...
    std::string line;
    // Each entry is "<ns>:<bytes>," — budget ~10 chars per entry.
    ...
    // Each entry is "<delta_ns>:<size_bytes>" so a slow outlier shows both how long
    // it took and how big the transfer was — a slow 1-byte store and a slow 32-byte
    // store have very different interpretations.
...
void memcpy_to_device(
    ...
    // timing record and the deadline check. op_bytes is the width of the
    // preceding store (1/4/16/32) — recorded with the delta so a slow op
    // can be correlated with its size.
    auto record_and_check = [&](std::chrono::steady_clock::time_point t_before, std::uint32_t op_bytes) {
        auto t_now = std::chrono::steady_clock::now();
        if (timing_enabled) {
            auto delta_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(t_now - t_before).count();
...
#if defined(__x86_64__) || defined(_M_X64)
    // The AVX2 intrinsics below are not themselves volatile. Each record_and_check() /
    // check_deadline() call is an opaque function call and therefore acts as a compiler
    // barrier — it prevents the compiler from reordering the volatile-conceptual SIMD
    // load/store around it. The explicit volatile loops in Phases 0/4/5 still bound the
    // ordering for the byte/word tails.
