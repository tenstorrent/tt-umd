...
// caller's on_timeout callback, with an overrun action that throws DeviceTimeoutError. `bytes_remaining`
// is captured by reference so the error reflects how much was left when the timeout fired. See
// device_memcpy.hpp for the on_timeout contract and its re-entrancy requirement.
