...
public:
    ...
    void start_child_process(const std::filesystem::path& simulator_directory,
    ...
    void send_message_with_response(MessageType type, const void* data, uint32_t data_size,
                                  void* response_data, uint32_t response_size);

    // Send a message with separate header and data to avoid copying (blocking)
    void send_message_with_data_and_response(MessageType type, const void* header_data, uint32_t header_size,
