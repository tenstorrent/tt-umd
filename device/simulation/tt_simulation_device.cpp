...
void tt_SimulationDevice::write_to_device_reg(
    const void* mem_ptr, uint32_t size_in_bytes, tt_cxy_pair core, uint64_t addr) {
    write_to_device(mem_ptr, size_in_bytes, core, addr);
}
...
void tt_SimulationDevice::read_from_device_reg(void* mem_ptr, tt_cxy_pair core, uint64_t addr, uint32_t size) {
...
void tt_SimulationDevice::read_from_device_reg(
    void* mem_ptr, chip_id_t chip, tt::umd::CoreCoord core, uint64_t addr, uint32_t size) {
    read_from_device(mem_ptr, {(size_t)chip, translate_to_api_coords(chip, core)}, addr, size);
}
