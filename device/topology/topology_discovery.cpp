...
std::unique_ptr<ClusterDescriptor> TopologyDiscovery::fill_cluster_descriptor_info() {
    ...
    for (auto [ethernet_connection_logical, ethernet_connection_remote] : ethernet_connections) {
        ...
        if (active_eth_channels_per_chip.at(local_asic_id).find(local_channel) == active_eth_channels_per_chip.at(local_asic_id).end()
            or active_eth_channels_per_chip.at(remote_asic_id).find(remote_channel) == active_eth_channels_per_chip.at(remote_asic_id).end()) {
                continue;
        }
        cluster_desc->ethernet_connections[local_chip_id][local_channel] = {
            remote_chip_id, remote_channel};
        cluster_desc->ethernet_connections[remote_chip_id][remote_channel] = {
            local_chip_id, local_channel};
    }
