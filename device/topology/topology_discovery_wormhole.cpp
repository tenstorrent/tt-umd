...
uint32_t TopologyDiscoveryWormhole::read_training_status(Chip* chip, tt_xy_pair eth_core) {
    ...
    TTDevice* tt_device = chip->get_tt_device();
    tt_device->read_from_device(&training_status, eth_core, 0x1104, sizeof(uint32_t), options.use_noc1);
...
std::optional<EthCoord> TopologyDiscoveryWormhole::get_local_eth_coord(Chip* chip, tt_xy_pair eth_core) {
    ...
    tt_device->read_from_device(
        &current_chip_eth_coord_info, eth_core, eth_addresses.node_info + 8, sizeof(uint32_t), options.use_noc1);
...
void TopologyDiscoveryWormhole::init_topology_discovery() {
    ...
    for (auto& device_id : device_ids) {
        ...
        tt_device->init_tt_device(options.use_noc1, timeout::ARC_LONG_POST_RESET_TIMEOUT);
    }
...
bool TopologyDiscoveryWormhole::verify_eth_core_fw_version(Chip* chip, CoreCoord eth_core) {
    ...
    if (options.verify_eth_fw_hash) {
        auto hash_check = verify_eth_fw_integrity(chip->get_tt_device(), eth_core, eth_fw_version, options.use_noc1);
...
bool TopologyDiscoveryWormhole::verify_routing_firmware_state(Chip* chip, const tt_xy_pair eth_core) {
    ...
    TTDevice* tt_device = chip->get_tt_device();
    tt_device->read_from_device(
        &routing_firmware_disabled, eth_core, eth_addresses.routing_firmware_state, sizeof(uint32_t), options.use_noc1);
