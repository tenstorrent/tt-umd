 ...
 */
 ...
 void TopologyDiscovery::discover_remote_chips() {
     ...
     while (!chips_to_discover.empty()) {
         ...
         std::vector<uint32_t> intermesh_eth_links = extract_intermesh_eth_links(chip);
         ...
         for (const CoreCoord& eth_core : eth_cores) {
             ...
             if (port_status == eth_unknown || port_status == eth_unconnected) {
                 if (std::find(intermesh_eth_links.begin(), intermesh_eth_links.end(), channel) != intermesh_eth_links.end()) {
                     if (is_intermesh_eth_link_trained(chip, eth_core)) {
                         std::cout<<"Intermesh ETH link found for chip " << current_chip_asic_id <<" channel " << channel <<" "<<"port_status: "<<port_status<< std::endl;
                     } else {
                         continue;
                     }
                 } else {
             ...
             if (!is_board_id_included(get_remote_board_id(chip, eth_core), get_remote_board_type(chip, eth_core))) {
                 std::cout<<"HERE"<<channel<<std::endl;
 ...
 std::vector<uint32_t> TopologyDiscovery::extract_intermesh_eth_links(Chip* chip) {
     static constexpr uint32_t INTERMESH_ETH_LINK_CONFIG_ADDR = 0x104C;
     static constexpr uint32_t INTERMESH_ETH_LINK_BITS_SHIFT = 8;
     static constexpr uint32_t INTERMESH_ETH_LINK_BITS_MASK = 0xFFFF;
     TTDevice* tt_device = chip->get_tt_device();
     std::vector<CoreCoord> eth_cores =
         chip->get_soc_descriptor().get_cores(CoreType::ETH, umd_use_noc1 ? CoordSystem::NOC1 : CoordSystem::NOC0);
     auto eth_core = eth_cores.at(0);
     uint32_t config_data;
     tt_device->read_from_device(&config_data, eth_core, INTERMESH_ETH_LINK_CONFIG_ADDR, sizeof(uint32_t));
 ...
 bool TopologyDiscovery::is_intermesh_eth_link_trained(Chip* chip, tt_xy_pair eth_core) {
     static constexpr std::uint32_t INTERMESH_ETH_LINK_STATUS_ADDR = 0x1104;
     static constexpr uint32_t LINK_CONNECTED_MASK = 0x1;
     uint32_t status;
     TTDevice* tt_device = chip->get_tt_device();
     tt_device->read_from_device(&status, eth_core, INTERMESH_ETH_LINK_STATUS_ADDR, sizeof(uint32_t));
