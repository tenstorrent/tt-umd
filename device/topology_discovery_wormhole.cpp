...
uint64_t TopologyDiscoveryWormhole::get_asic_id(Chip* chip) {
...
eth_coord_t TopologyDiscoveryWormhole::get_local_eth_coord(Chip* chip) {
    std::vector<CoreCoord> eth_cores =
        chip->get_soc_descriptor().get_cores(CoreType::ETH, umd_use_noc1 ? CoordSystem::NOC1 : CoordSystem::NOC0);
    TTDevice* tt_device = chip->get_tt_device();
...
eth_coord_t TopologyDiscoveryWormhole::get_remote_eth_coord(Chip* chip, tt_xy_pair eth_core) {
    const uint32_t shelf_offset = 9;
...
void TopologyDiscoveryWormhole::init_topology_discovery() {
    ...
    eth_addresses = TopologyDiscoveryWormhole::get_eth_addresses(
        tt_device->get_arc_telemetry_reader()->read_entry(wormhole::TAG_ETH_FW_VERSION));
}
