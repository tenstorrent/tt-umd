...
void PCIeCommunication::write_block(uint64_t byte_addr, uint64_t num_bytes, const uint8_t *buffer_addr) {
    ...
    if (architecture_implementation.get_architecture() == tt::ARCH::WORMHOLE_B0) {
...
void PCIeCommunication::write_regs(volatile uint32_t *dest, const uint32_t *src, uint32_t word_len) {
    while (word_len-- != 0) {
        *dest++ = *src++;
    }
}
...
void PCIeCommunication::write_tlb_reg(
...
dynamic_tlb PCIeCommunication::set_dynamic_tlb(
    ...
    tlb_configuration tlb_config = architecture_implementation.get_tlb_configuration(tlb_index);
    ...
    auto arch = architecture_implementation.get_architecture();
...
dynamic_tlb PCIeCommunication::set_dynamic_tlb_broadcast(
...
void PCIeCommunication::memcpy_to_device(void *dest, const void *src, std::size_t num_bytes) {
...
void PCIeCommunication::memcpy_from_device(void *dest, const void *src, std::size_t num_bytes) {
    typedef std::uint32_t copy_t;
