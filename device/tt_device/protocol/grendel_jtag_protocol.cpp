...
void GrendelJtagProtocol::write_data(const void* src, tt_xy_pair core, uint64_t addr, size_t size, NocId noc_id) {
    ...
    try {
        ...
        impl_->transport_for(core)->write(size, kMinWordSizeBytes, addr, const_cast<void*>(src));
...
void GrendelJtagProtocol::read_data(void* dst, tt_xy_pair core, uint64_t addr, size_t size, NocId noc_id) {
    if (noc_id != NocId::NOC0) {
        UMD_THROW(
            error::RuntimeError, fmt::format("GrendelJtagProtocol supports only NOC0 (got {}).", noc_to_str(noc_id)));
    }
    try {
        impl_->transport_for(core)->read(size, kMinWordSizeBytes, addr, dst);
    } catch (const std::exception& e) {
        UMD_THROW(
            error::RuntimeError,
            fmt::format(
...
void GrendelJtagProtocol::read_ctrl(void* dst, tt_xy_pair core, uint64_t addr, size_t size, NocId noc_id) {
    validate_register_access(addr, size);
    read_data(dst, core, addr, size, noc_id);
}

void GrendelJtagProtocol::write_ctrl(const void* src, tt_xy_pair core, uint64_t addr, size_t size, NocId noc_id) {
    validate_register_access(addr, size);
