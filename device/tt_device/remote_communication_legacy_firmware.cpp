...
RemoteCommunicationLegacyFirmware::RemoteCommunicationLegacyFirmware(
    ...
    RemoteCommunication(mmio_protocol, sysmem_manager), target_chip(target_chip) {}
...
void RemoteCommunicationLegacyFirmware::read_non_mmio(
    ...
    auto lock = lock_manager_.acquire_mutex(MutexType::NON_MMIO, mmio_protocol_->get_communication_device_id());

    using data_word_t = uint32_t;
    constexpr int DATA_WORD_SIZE = sizeof(data_word_t);

    auto host_address_params = mmio_protocol_->get_architecture_implementation()->get_host_address_params();
    auto eth_interface_params = mmio_protocol_->get_architecture_implementation()->get_eth_interface_params();
    auto noc_params = mmio_protocol_->get_architecture_implementation()->get_noc_params();
    ...
    mmio_protocol_->read_from_device(
        erisc_q_ptrs.data(),
    ...
    while (offset < size_in_bytes) {
        ...
        mmio_protocol_->write_to_device(
            erisc_command.data(),
            remote_transfer_ethernet_core,
...
void RemoteCommunicationLegacyFirmware::wait_for_non_mmio_flush(const std::chrono::milliseconds timeout_ms) {
    if (flush_non_mmio_) {
        TT_ASSERT(mmio_protocol_->get_arch() != tt::ARCH::BLACKHOLE, "Non-MMIO flush not supported in Blackhole");

        if (mmio_protocol_->get_arch() == tt::ARCH::WORMHOLE_B0) {
            auto eth_interface_params = mmio_protocol_->get_architecture_implementation()->get_eth_interface_params();
