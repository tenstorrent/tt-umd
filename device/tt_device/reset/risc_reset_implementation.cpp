...
RiscResetImplementation::RiscResetImplementation(
    ...
    UMD_ASSERT(
        ...
        error::RuntimeError,
        "RiscResetImplementation requires a DeviceProtocol and an ArchitectureImplementation.");
}

void RiscResetImplementation::assert_risc_reset(tt_xy_pair core, RiscType selected_riscs, NocId noc_id) {
...
void RiscResetImplementation::deassert_risc_reset(
...
std::optional<RiscType> RiscResetImplementation::get_risc_reset_state(tt_xy_pair core, NocId noc_id) {
    return architecture_impl_->get_soft_reset_risc_type(read_reset_register(core, noc_id));
}

uint32_t RiscResetImplementation::read_reset_register(tt_xy_pair core, NocId noc_id) {
    uint32_t value = 0;
    device_protocol_->read_ctrl(&value, core, architecture_impl_->get_tensix_soft_reset_addr(), sizeof(value), noc_id);
    return value;
}

void RiscResetImplementation::write_reset_register(tt_xy_pair core, uint32_t value, NocId noc_id) {
