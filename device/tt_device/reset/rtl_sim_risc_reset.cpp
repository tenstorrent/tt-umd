...
// The DM cores the backend resets one at a time, in the order its commands index them.
constexpr std::array<RiscType, 8> RISC_TYPES_DMS = {
    RiscType::DM0,
    RiscType::DM1,
    RiscType::DM2,
    RiscType::DM3,
...
void RtlSimRiscReset::assert_risc_reset(tt_xy_pair core, RiscType selected_riscs, [[maybe_unused]] NocId noc_id) {
    UMD_ASSERT(
        communicator_ != nullptr, error::RuntimeError, "RISC reset is not available for a client-mode RTL device.");
...
std::optional<RiscType> RtlSimRiscReset::get_risc_reset_state(
