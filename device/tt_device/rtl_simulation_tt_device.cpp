...
RtlSimulationTTDevice::RtlSimulationTTDevice(
    ...
    tlb_allocator_ = std::make_unique<SimulationTlbAllocator>(/*bar0_base=*/0, architecture_impl_.get());
    ...
    switch (arch) {
        ...
        case tt::ARCH::QUASAR: {
            ...
            auto handle = RtlSimTlbHandle::create(tlb_allocator_.get(), 0, SIZE_4GB, TlbMapping::WC);
            cached_tlb_window_ = std::make_unique<RtlSimTlbWindow>(std::move(handle), communicator_.get(), tlb_data{});
            break;
        }
        default:
            log_debug(
...
std::unique_ptr<IOWindow> RtlSimulationTTDevice::get_io_window(tlb_data config, TlbMapping mapping, size_t size) {
    ...
    auto handle = RtlSimTlbHandle::create(tlb_allocator_.get(), tlb_index, actual_size, mapping);
