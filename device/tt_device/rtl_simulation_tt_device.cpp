...
bool RtlSimulationTTDevice::should_use_cached_tlb_window() {
    // A resolved address already names its destination, so the dummy window Quasar would allocate
    // to carry that coordinate in tlb_data has nothing left to carry.
    return noc_address_resolver_ == nullptr && cached_tlb_window_ != nullptr;
}
