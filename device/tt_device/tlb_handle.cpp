...
TlbHandle::TlbHandle(PCIDevice* pci_device, size_t size, const tenstorrent_noc_tlb_config& config) :
    ...
    try {
        ...
        void* wc = mmap(
            nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, pci_device->get_fd(), allocate_tlb_out.mmap_offset_wc);
        if (wc == MAP_FAILED) {
            munmap(uc, size);
            throw std::runtime_error("Failed to map TLB WC base using mmap call.");
        }
...
uint8_t* TlbHandle::get_base_uc() { return tlb_base_uc; }

uint8_t* TlbHandle::get_base_wc() { return tlb_base_wc; }
