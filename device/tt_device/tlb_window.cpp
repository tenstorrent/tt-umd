/*
 * SPDX-FileCopyrightText: (c) 2025 Tenstorrent Inc.
 *
 * SPDX-License-Identifier: Apache-2.0
 */
#include "umd/device/tt_device/tlb_window.h"

namespace tt::umd {

TlbWindow::TlbWindow(std::unique_ptr<TlbHandle> handle) : handle(std::move(handle)) {}

// Memory access with selectable caching
void TlbWindow::write32(uint64_t offset, uint32_t value, CacheMode mode) {
    validate(offset, sizeof(uint32_t));
    *reinterpret_cast<volatile uint32_t*>(base(mode) + offset) = value;
}

uint32_t TlbWindow::read32(uint64_t offset, CacheMode mode) {
    validate(offset, sizeof(uint32_t));
    return *reinterpret_cast<volatile uint32_t*>(base(mode) + offset);
}

// Register access (always 32-bit, always uncached)
void TlbWindow::write_register(uint64_t offset, uint32_t value) { write32(offset, value, CacheMode::UC); }

uint32_t TlbWindow::read_register(uint64_t offset) { return read32(offset, CacheMode::UC); }

// Block transfers are limited to 32-bit aligned sizes.
void TlbWindow::write_block(uint64_t offset, const void* data, size_t size, CacheMode mode) {
    size_t n = size / sizeof(uint32_t);
    auto* src = static_cast<const uint32_t*>(data);
    auto* dst = reinterpret_cast<volatile uint32_t*>(base(mode) + offset);

    validate(offset, size);

    for (size_t i = 0; i < n; i++) {
        dst[i] = src[i];
    }
}

void TlbWindow::read_block(uint64_t offset, void* data, size_t size, CacheMode mode) {
    size_t n = size / sizeof(uint32_t);
    auto* src = reinterpret_cast<const volatile uint32_t*>(base(mode) + offset);
    auto* dst = static_cast<uint32_t*>(data);

    validate(offset, size);

    for (size_t i = 0; i < n; i++) {
        dst[i] = src[i];
    }
}

TlbHandle& TlbWindow::handle_ref() { return *handle; }

size_t TlbWindow::get_size() const { return handle->get_size(); }

// For simplicity and correctness, only allow 32-bit aligned accesses.
// There exist platform and device specific considerations for unaligned
// accesses which are not addressed here.
void TlbWindow::validate(uint64_t offset, size_t size) const {
    if ((offset + size) > get_size()) {
        throw std::out_of_range("Out of bounds access");
    }

    if (offset & (sizeof(uint32_t) - 1)) {
        throw std::runtime_error("Bad alignment");
    }
}

uint8_t* TlbWindow::base(CacheMode mode) {
    return (mode == CacheMode::UC ? handle->get_base_uc() : handle->get_base_wc());
}

}  // namespace tt::umd
