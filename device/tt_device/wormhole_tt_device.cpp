...
std::unique_ptr<DeviceFirmware> WormholeTTDevice::create_device_firmware() {
    // Each of these is null unless the device is reached that way; a non-null RemoteInterface is also
    // what tells the firmware to route ARC accesses over the NOC and to flush ethernet writes.
    const bool is_jtag = get_communication_device_type() == IODeviceType::JTAG;
    return std::make_unique<WormholeDeviceFirmware>(
        get_device_protocol(),
        is_jtag || is_remote() ? nullptr : get_pcie_interface(),
        is_jtag ? get_jtag_interface() : nullptr,
        is_remote() ? get_remote_interface() : nullptr,
        get_architecture_implementation(),
