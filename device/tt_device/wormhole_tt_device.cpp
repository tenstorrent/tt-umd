...
void WormholeTTDevice::wait_eth_core_training(const tt_xy_pair eth_core, const uint32_t timeout_ms) {
    ...
    uint32_t eth_channel = std::distance(
        wormhole::ETH_CORES_NOC0.begin(),
        std::find(wormhole::ETH_CORES_NOC0.begin(), wormhole::ETH_CORES_NOC0.end(), eth_core));

    uint32_t port_status = read_port_status(eth_core, eth_channel);
    start = std::chrono::system_clock::now();
    while (port_status == ETH_UNKNOWN) {
        port_status = read_port_status(eth_core, eth_channel);
        auto end = std::chrono::system_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        if (duration.count() > timeout_ms) {
...
uint32_t WormholeTTDevice::read_port_status(tt_xy_pair eth_core, uint32_t channel) {
    EthAddresses eth_addresses = get_eth_addresses();
    uint32_t port_status;
    read_from_device(&port_status, eth_core, eth_addresses.eth_conn_info + (channel * 4), sizeof(uint32_t));
    return port_status;
}

WormholeTTDevice::EthAddresses WormholeTTDevice::get_eth_addresses() {
    uint32_t eth_fw_version = telemetry->read_entry(wormhole::TAG_ETH_FW_VERSION);
