...
void tt_SiliconDevice::write_to_non_mmio_device(
    ...
    while (offset < size_in_bytes) {
        ...
        tt_device_logger::log_assert(broadcast || (req_flags == eth_interface_params.CMD_WR_REQ) || (((address + offset) % 32) == 0), "Block mode address must be 32-byte aligned."); // Block mode address must be 32-byte aligned.
        ...
        for(const auto& core : cores) {
            if (cores_synced.find(core) == cores_synced.end()) {
                ...
                if (readback_vec.at(0) == barrier_value) {
                    ...
                    tt_device_logger::log_info(tt_device_logger::LogSiliconDriver, "Waiting for core {} to recieve mem bar flag {} in function", core.str(), barrier_value, __FUNCTION__);
                }
            }
        }
    }
