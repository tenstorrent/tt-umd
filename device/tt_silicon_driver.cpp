...
void tt_SiliconDevice::write_to_non_mmio_device(
    ...
    while (offset < size_in_bytes) {
        ...
        log_assert(broadcast || (req_flags == eth_interface_params.CMD_WR_REQ) || (((address + offset) & 0x1F) == 0), LogSiliconDriver, "Block mode address must be 32-byte aligned."); // Block mode address must be 32-byte aligned.
...
void tt_SiliconDevice::write_to_non_mmio_device_send_epoch_cmd(const uint32_t *mem_ptr, uint32_t size_in_bytes, tt_cxy_pair core, uint64_t address, bool last_send_epoch_cmd) {
    ...
    // log_debug(LogDevice, "Writing to non-mmio device {}: tt_cxy_pair {}, addr {}", target_chip.str(), core.str(), address);
        ...
        for(const auto& core : cores) {
            if (cores_synced.find(core) == cores_synced.end()) {
                ...
                if (readback_vec.at(0) == barrier_value) {
                    ...
                    log_debug(tt::LogSiliconDriver, "Waiting for core {} to recieve mem bar flag {} in function", core.str(), barrier_value, __FUNCTION__);
                }
            }
        }
    }
