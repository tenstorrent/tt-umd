...
std::uint32_t tt_SiliconDevice::get_num_host_channels(std::uint32_t device_id) {
    auto mmio_devices = get_target_mmio_device_ids();
    log_assert(mmio_devices.find(device_id) != mmio_devices.end(), "Querying Host Address parameters for a non-mmio device or a device does not exist.");
