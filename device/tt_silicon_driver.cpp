...
tt_SiliconDevice::tt_SiliconDevice(const std::string &sdesc_path, const std::string &ndesc_path, const std::set<chip_id_t> &target_devices, 
    ...
    if(simulated_harvesting_masks.size()) {
        performed_harvesting = true;
        tt_device_logger::log_assert(simulated_harvesting_masks.size() == target_devices.size(),
                    ...
                    target_devices.size(), simulated_harvesting_masks.size());
        for (auto device_id = target_devices.begin(); device_id != target_devices.end(); device_id++) {
            if(arch_name == tt::ARCH::GRAYSKULL) {
                tt_device_logger::log_assert((simulated_harvesting_masks.at(*device_id) & harvested_rows_per_target[*device_id]) == harvested_rows_per_target[*device_id],
                ...
                tt_device_logger::log_assert(std::bitset<32>(simulated_harvesting_masks.at(*device_id)).count() >= std::bitset<32>(*device_id).count(),
...
std::unordered_map<chip_id_t, uint32_t> tt_SiliconDevice::get_harvesting_masks(std::unordered_map<chip_id_t, std::vector<uint32_t>> harvested_rows) {
