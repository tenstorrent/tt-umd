...
// use forward declaration here so we do not need to include
class tt_vcs_wrapper;


class tt_simulation_device : public tt_device {
public:
  virtual void set_device_l1_address_params(const tt_device_l1_address_params& l1_address_params_); // Dont care
  tt_simulation_device(const std::string& sdesc_path);
...
private:
  ...
  tt_vcs_wrapper *tt_vcs_wrapper_inst = NULL;
