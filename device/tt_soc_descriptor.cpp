...
tt_xy_pair tt_SocDescriptor::calculate_grid_size(const std::vector<tt_xy_pair> &cores) {
    ...
    for (size_t i = 1; i < cores.size(); i++) {
        if (cores[i].y == cores[i - 1].y) {
            curr_x++;
        } else {
            if (x != 0 && curr_x != x) {
                throw std::runtime_error("Every NOC row must have the same number of cores.");
            }
            curr_x = 1;
            y++;
        }
    ...
    // Special case to account for the case when there is
    // only one row so x won't get updated.
    if (x == 0) {
        x = curr_x;
    }

    // To account for the last row.
    y++;
