...
public:
  ...
  virtual void rolled_write_to_device(const std::vector<uint32_t>& base_vec, uint32_t unroll_count, tt_cxy_pair core, uint64_t base_addr, const std::string& tlb_to_use); // See Versim Implementation
  ...
  virtual ~tt_vcs_device(); // Should close the FIFOs
