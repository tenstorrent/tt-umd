...
pthread_mutex_t RobustSystemMutex::multithread_mutex_ = PTHREAD_MUTEX_INITIALIZER;

RobustSystemMutex::RobustSystemMutex(std::string_view mutex_name) : mutex_name_(mutex_name) {}

RobustSystemMutex::~RobustSystemMutex() noexcept { close_mutex(); }

RobustSystemMutex::RobustSystemMutex(RobustSystemMutex&& other) noexcept {
...
RobustSystemMutex& RobustSystemMutex::operator=(RobustSystemMutex&& other) noexcept {
    if (this != &other) {
...
void RobustSystemMutex::initialize() {
    open_shm_file();
...
void RobustSystemMutex::open_shm_file() {
...
bool RobustSystemMutex::resize_shm_file() {
...
void RobustSystemMutex::open_pthread_mutex() {
...
void RobustSystemMutex::initialize_pthread_mutex_first_use() {
    int err;
...
size_t RobustSystemMutex::get_file_size(int fd) {
    struct stat sb;
    TT_ASSERT(fstat(fd, &sb) == 0, "fstat failed for mutex {} errno: {}", mutex_name_, std::to_string(errno));
    return sb.st_size;
}

void RobustSystemMutex::close_mutex() noexcept {
