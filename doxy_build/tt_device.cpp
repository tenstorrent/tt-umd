...
void TTDevice::init_device(std::chrono::milliseconds timeout_ms) {
    ...
    device_controller_ = model_->create_device_controller();
    ...
    soc_descriptor_.emplace(std::make_shared<SocArchDescriptor>(arch_), device_controller_->get_chip_info());
}
...
void TTDevice::dma_write_to_core_range(const void *src, uint64_t dst_addr, size_t size, CoreCoord core) {
    auto res = dma_interface_->dma_write(src, dst_addr, size, translate(core), NocId::DEFAULT_NOC);
    if (!res) {
        // fallback
    }
}
...
void TTDevice::dma_read(void *dst, uint64_t src_addr, size_t size, CoreCoord core) {
    auto res = dma_interface_->dma_read(dst, src_addr, size, translate(core), NocId::DEFAULT_NOC);
    if (!res) {
        // fallback
    }
}
...
std::chrono::milliseconds TTDevice::wait_eth_core_training(CoreCoord eth_core, std::chrono::milliseconds timeout_ms) {
    return device_controller_->wait_eth_core_training(eth_core, timeout_ms);
}

void TTDevice::wait_dram_channel_training(uint32_t dram_channel, std::chrono::milliseconds timeout_ms) {
    device_controller_->wait_dram_channel_training(dram_channel, timeout_ms);
}
...
EthTrainingStatus TTDevice::read_eth_core_training_status(CoreCoord eth_core) {
    return device_controller_->get_eth_core_training_status(eth_core);
}

void TTDevice::set_power_state(PowerState state) { device_firmware_->set_power_state(static_cast<uint32_t>(state)); }

void TTDevice::set_clock_state(PowerState state) { device_controller_->set_clock_state(static_cast<uint32_t>(state)); }
...
ChipInfo TTDevice::get_chip_info() { return device_controller_->get_chip_info(); }
...
uint32_t TTDevice::get_clock_freq() const { return firmware_info_provider_->get_clock_freq(); }
