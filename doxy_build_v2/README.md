...
| Document | Content |
|----------|---------|
| `chip_tt_device_mapping_1.0.pdf` | Chip / LocalChip / RemoteChip → TTDevice dependency mapping. Per-device workload layer as used by tt-metal. |
| `cluster_tt_device_mapping_1.0.pdf` | Cluster → TTDevice dependency mapping. Multi-device orchestration layer as used by tt-metal. |
| `tt_exalens_tt_umd_mapping_1.0.pdf` | tt-exalens → UMD base API mapping. Debugger integration through Python nanobind bindings. |
...
| Document | Content |
|----------|---------|
| `base_tt_device_reference_1.0.pdf` | TTDevice API reference with full header and implementation source. Covers coordinate translation, DMA fallback logic, firmware delegation, hang detection, etc. |
...
| Document | Content |
|----------|---------|
| `base_components_umd_1.0.pdf` | API reference for all base layer component interfaces: DeviceProtocol, DeviceFirmware, ArchitectureImplementation, IoWindow, FirmwareInfoProvider, DmaInterface, HangDetector, PcieInterface, JtagInterface, RemoteInterface, SystemMemory, TopologyDiscovery, SocArchDescriptor, SocDescriptor. Includes shared type definitions and constants. |
