...
public:
    ...
    virtual TLBWindow get_tlb_window(TLBType type) = 0;
...
public:
    StandardTLBManager(TTDevice* tt_device) : tt_device(tt_device) {}

    TLBWindow get_tlb_window(tt_xy_pair core) override {
        uint32_t tlb_index = tt_device->map_core_to_tlb_index(core);
    ...
    TLBWindow get_tlb_window(TLBType type) override {
        if (type == TLBType::WC) {
            return get_wc_tlb_window();
        } else if (type == TLBType::UC) {
            return get_uc_tlb_window();
        }
    }
    ...
    TLBWindow get_dram_tlb_widow(uint32_t dram_channel, uint32_t addr = 0) override {
        ...
        return get_tlb_window(tt_xy_pair(0, 0));
    }
...
private:
    ...
    TLBWindow get_tlb_window(uint32_t tlb_index) {
        if (!tt_device->pci_device->bar0_wc) {
            throw std::runtime_error("No write-combined mapping for BAR0");
        }

        auto tlb_data = tt_device->pci_device->describe_tlb(tlb_index);
        ...
        uint8_t* base = reinterpret_cast<uint8_t*>(tt_device->pci_device->bar0_wc);
