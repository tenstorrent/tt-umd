...
void bind_tt_device(nb::module_ &m) {
    ...
    nb::class_<TTDevice>(m, "TTDevice")
        ...
        .def(
            ...
            "coordinates. core_type must be CoreType.TENSIX or CoreType.ETH.")
        .def(
            "l1_membar",
            ...
            nb::arg("cores"),
            nb::arg("barrier_address"),
            "Insert a memory barrier on the specified L1 TENSIX cores. Cores should be a list of (x, y) tuples in "
            "translated coordinates. Defaults to CoreType.TENSIX.")
        .def(
            "arc_msg",
            [](TTDevice &self,
               uint32_t msg_code,
