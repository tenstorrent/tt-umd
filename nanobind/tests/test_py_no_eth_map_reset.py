    ...
    def test_profiling_and_reset(self):
        ...
        for chip_id in cluster_desc.get_all_chips():
            device_map[chip_id] = {
                ...
                "closest_mmio_chip": (
                    cluster_desc.get_closest_mmio_capable_chip(chip_id)
                    if not cluster_desc.is_chip_mmio_capable(chip_id)
                    else chip_id
                ),
                # Location
                "location": cluster_desc.get_chip_locations().get(
                    chip_id
                ),  # EthCoord with cluster_id, x, y, rack, shelf
                # Board info
                "board_type": cluster_desc.get_board_type(chip_id),
                "board_id": (
                    cluster_desc.get_board_id_for_chip(chip_id)
                    if chip_id in cluster_desc.get_chips_with_mmio()
                    else None
                ),
                # Ethernet
                "active_eth_channels": list(
                    cluster_desc.get_active_eth_channels(chip_id)
                ),
                "ethernet_connections": cluster_desc.get_ethernet_connections().get(
                    chip_id, {}
                ),
            }
        ...
        for chip_id, info in device_map.items():
            ...
            if info["location"]:
                loc = info["location"]
                print(
                    f"  Location: cluster={loc.cluster_id}, rack={loc.rack}, shelf={loc.shelf}, x={loc.x}, y={loc.y}"
                )

        print(f"\n{'='*60}")
        print(f"TOTAL TIME BREAKDOWN:")
        print(
            f"  Test 1 (Full discovery):                   Reset: {reset_time_1:.3f}s, Discovery: {time_1:.3f}s"
        )
        print(
            f"  Test 2 (Local-only, full initialization):  Reset: {reset_time_2:.3f}s, Discovery: {time_2:.3f}s"
        )
        print(
            f"  Test 3 (Local-only, no waiting on eth):    Reset: {reset_time_3:.3f}s, Discovery: {time_3:.3f}s"
        )
