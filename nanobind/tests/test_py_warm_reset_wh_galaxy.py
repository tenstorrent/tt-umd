...
class TestWarmReset(unittest.TestCase):
    ...
    def test_warm_reset(self):
        ...
        # Skip test if not a Wormhole device
        if arch != tt_umd.ARCH.WORMHOLE_B0:
            self.skipTest(f"Test only runs on Wormhole devices, found: {arch}")
        ...
        if pci_devices_info[0].subsystem_id == 0x0035:
            print(f"Executing standard warm reset with secondary bus reset disabled...")
            tt_umd.WarmReset.warm_reset(pci_ids, secondary_bus_reset=False)  
        else:
            print(f"Not a UBB device, skipping warm reset...")
