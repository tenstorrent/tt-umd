...
const std::filesystem::path cluster_descriptor_file_path = tt::umd::repo_root / "clustermap.json";

#if defined(__x86_64__) || defined(__i386__)
    const std::filesystem::path eth_fpath = tt::umd::repo_root / "device/bin/silicon/x86/create-ethernet-map";
#elif defined(__aarch64__) || defined(_M_ARM64)
    const std::filesystem::path eth_fpath = tt::umd::repo_root / "device/bin/silicon/aarch64/create-ethernet-map";
#else
...
std::unique_ptr<ClusterDescriptor> ClusterDescriptor::cluster_descriptor;
...
ClusterDescriptor *ClusterDescriptor::GetClusterDescriptor(bool should_create_for_grayskull_cluster) {
    if (cluster_descriptor == nullptr) {
        if (should_create_for_grayskull_cluster) {
            // brosko: yeah this is bad, move discovery somewhere
            std::vector<chip_id_t> physical_mmio_device_ids;
            std::set<chip_id_t> logical_mmio_device_ids;
            physical_mmio_device_ids = ttkmd_scan();
            ...
            cluster_descriptor = create_for_grayskull_cluster(logical_mmio_device_ids, physical_mmio_device_ids);
        } else {
            GenerateClusterMap();
            cluster_descriptor = create_from_yaml(cluster_descriptor_file_path);
        }
    }

    return cluster_descriptor.get();
}

void ClusterDescriptor::GenerateClusterMap() {
    // Generates the cluster descriptor in the CWD
    std::string cmd = eth_fpath.string() + " " + cluster_descriptor_file_path.string();
    int val = system(cmd.c_str());
    if (val != 0)
        throw std::runtime_error("Cluster Generation with create-ethernet-map Failed!");
}

std::unique_ptr<ClusterDescriptor> ClusterDescriptor::create_from_yaml(std::filesystem::path cluster_descriptor_yaml_file) {
    std::unique_ptr<ClusterDescriptor> desc = std::unique_ptr<ClusterDescriptor>(new ClusterDescriptor());

    std::ifstream fdesc(cluster_descriptor_yaml_file);
    if (fdesc.fail()) {
        throw std::runtime_error(
            "Error: cluster connectivity descriptor file " + cluster_descriptor_yaml_file.string() + " does not exist!");
    }
    fdesc.close();
...
void ClusterDescriptor::load_ethernet_connections_from_connectivity_descriptor(YAML::Node &yaml) {
    ...
    for (YAML::Node &connected_endpoints : yaml["ethernet_connections"].as<std::vector<YAML::Node>>()) {
        ...
        if (ethernet_connections[chip_0].find(channel_0) != ethernet_connections[chip_0].end()) {
            log_assert(
                (std::get<0>(ethernet_connections[chip_0][channel_0]) == chip_1) &&
                    (std::get<1>(ethernet_connections[chip_0][channel_0]) == channel_1),
                "Duplicate eth connection found in cluster desc yaml");
        } else {
            ethernet_connections[chip_0][channel_0] = {chip_1, channel_1};
        }
        if (ethernet_connections[chip_1].find(channel_1) != ethernet_connections[chip_0].end()) {
            log_assert(
                (std::get<0>(ethernet_connections[chip_1][channel_1]) == chip_0) &&
                    (std::get<1>(ethernet_connections[chip_1][channel_1]) == channel_0),
                "Duplicate eth connection found in cluster desc yaml");
        } else {
            ethernet_connections[chip_1][channel_1] = {chip_0, channel_0};
        }
    }
    ...
    for (const auto &[chip_id, chip_eth_coord] : chip_locations) {
        ...
        for (const auto &[chan, chip_and_chan] : ethernet_connections.at(chip_id)) {
            ...
            eth_coord_t neighbor_eth_coord = chip_locations.at(neighbor_chip);
            ...
            if (std::get<2>(neighbor_eth_coord) != std::get<2>(chip_eth_coord)) {
                ...
                auto &galaxy_rack_exit_chip_coords_per_x_dim =
                    galaxy_racks_exit_chip_coords_per_x_dim[lower_rack_id];
    ...
    for (int shelf_id = 0; shelf_id < highest_shelf_id; shelf_id++) {
        log_assert(
            galaxy_shelves_exit_chip_coords_per_y_dim.find(shelf_id) !=
    ...
    for (int rack_id = 0; rack_id < highest_rack_id; rack_id++) {
        log_assert(
            galaxy_racks_exit_chip_coords_per_x_dim.find(rack_id) !=
...
void ClusterDescriptor::load_chips_from_connectivity_descriptor(YAML::Node &yaml) {
    for (YAML::const_iterator node = yaml["chips"].begin(); node != yaml["chips"].end(); ++node) {
        ...
        coords_to_chip_ids[std::get<2>(chip_location)][std::get<3>(chip_location)][std::get<1>(chip_location)]
    ...
    if (yaml["boardtype"]) {
        for (const auto &chip_board_type_yaml_node : yaml["boardtype"].as<std::map<int, std::string>>()) {
            auto &chip = chip_board_type_yaml_node.first;
            BoardType board_type;
            if (chip_board_type_yaml_node.second == "n150") {
                board_type = BoardType::N150;
            } else if (chip_board_type_yaml_node.second == "n300") {
                board_type = BoardType::N300;
            } else if (chip_board_type_yaml_node.second == "GALAXY") {
        ...
        for (const auto &chip : all_chips) {
            chip_board_type.insert({chip, BoardType::DEFAULT});
        }
    }
}
...
std::unique_ptr<ClusterDescriptor> ClusterDescriptor::create_for_grayskull_cluster(
...
void ClusterDescriptor::enable_all_devices() { this->enabled_active_chips = this->all_chips; }
...
std::unordered_map<chip_id_t, eth_coord_t> ClusterDescriptor::get_chip_locations() const {
...
std::unordered_map<chip_id_t, chip_id_t> ClusterDescriptor::get_chips_with_mmio() const {
...
std::unordered_set<chip_id_t> ClusterDescriptor::get_all_chips() const { return this->enabled_active_chips; }

std::unordered_map<chip_id_t, std::uint32_t> ClusterDescriptor::get_harvesting_info() const { return harvesting_masks; }

std::unordered_map<chip_id_t, bool> ClusterDescriptor::get_noc_translation_table_en() const {
    return noc_translation_enabled;
}

std::size_t ClusterDescriptor::get_number_of_chips() const { return this->enabled_active_chips.size(); }
