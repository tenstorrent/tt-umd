...
uint32_t get_available_num_host_mem_channels(const uint32_t num_channels_per_device_target, const uint16_t device_id, const uint16_t revision_id) {
    ...
    uint32_t total_num_tt_mmio_devices      = cpuset::tt_cpuset_allocator::get_num_tt_pci_devices();
    uint32_t num_tt_mmio_devices_for_arch   = cpuset::tt_cpuset_allocator::get_num_tt_pci_devices_by_pci_device_id(device_id, revision_id);
...
bool LocalChip::address_in_tlb_space(
...
SocDescriptor& LocalChip::get_soc_descriptor(chip_id_t chip_id) { return soc_descriptor_per_chip.at(chip_id); }

std::unordered_map<chip_id_t, SocDescriptor>& LocalChip::get_virtual_soc_descriptors() {
    return soc_descriptor_per_chip;
}

void LocalChip::initialize_interprocess_mutexes(int pci_interface_id, bool cleanup_mutexes_in_shm) {
    ...
    if (arch_name == Arch::WORMHOLE or arch_name == Arch::WORMHOLE_B0) {
...
void LocalChip::create_device(
    ...
    for (const chip_id_t& logical_device_id : target_mmio_device_ids) {
        ...
        m_pci_device_map.insert({logical_device_id, TTDevice::open(pci_interface_id)});
        TTDevice* tt_device = m_pci_device_map.at(logical_device_id).get();
        ...
        // brosko: this is put some other place, revisit
        // pci_device->logical_id = logical_device_id;

        m_num_host_mem_channels = get_available_num_host_mem_channels(
            num_host_mem_ch_per_mmio_device, tt_device->device_id, tt_device->pci_device->get_revision_id());
        if (arch_name == Arch::BLACKHOLE && m_num_host_mem_channels > 1) {
        ...
        log_debug(
            LogSiliconDriver,
            "Using {} Hugepages/NumHostMemChannels for TTDevice (logical_device_id: {} pci_interface_id: {} device_id: "
            "0x{:x} revision: {})",
            m_num_host_mem_channels,
            logical_device_id,
            pci_interface_id,
            tt_device->pci_device->device_id,
            tt_device->pci_device->get_revision_id());

        if (g_SINGLE_PIN_PAGE_PER_FD_WORKAROND) {
            tt_device->pci_device->open_hugepage_per_host_mem_ch(m_num_host_mem_channels);
        }
        ...
        archs_in_cluster.push_back(tt_device->pci_device->arch);
    }
...
bool LocalChip::using_harvested_soc_descriptors() { return perform_harvesting_on_sdesc && performed_harvesting; }

std::unordered_map<xy_pair, xy_pair> LocalChip::get_harvested_coord_translation_map(chip_id_t logical_device_id) {
    return harvested_coord_translation.at(logical_device_id);
}

std::unordered_map<chip_id_t, uint32_t> LocalChip::get_harvesting_masks_for_soc_descriptors() {
...
LocalChip::LocalChip(
    ...
    // brosko remove clusterdescriptor from chip altogether
    // if (ndesc_path == "") {
    //     ndesc = ClusterDescriptor::create_for_grayskull_cluster(target_devices, available_device_ids);
    // } else {
        ndesc = std::shared_ptr<ClusterDescriptor>(ClusterDescriptor::GetClusterDescriptor());
    // }
    ...
    arch_name = m_pci_device_map[0]->pci_device->arch;
    ...
    // writes.
    // auto architecture_implementation =
    //     tt::umd::architecture_implementation::create(static_cast<tt::umd::architecture>(arch_name));
    // brosko: I've changed this, might be something wrong here
    dynamic_tlb_config["LARGE_READ_TLB"] = m_pci_device_map.begin()->second->get_mem_large_read_tlb();
    dynamic_tlb_config["LARGE_WRITE_TLB"] = m_pci_device_map.begin()->second->get_mem_large_write_tlb();
        ...
        for (auto device_id = target_devices.begin(); device_id != target_devices.end(); device_id++) {
            ...
            if (arch_name == Arch::GRAYSKULL) {
                log_assert(
                    ...
                    "Simulated harvesting config for device {} does not include the actual harvesting config (real "
                    "config must be contained in simulated config when running on device). Actual Harvested Rows : {}  "
                    "  Simulated Harvested Rows : {}",
                    *device_id,
                    harvested_rows_per_target[*device_id],
                    simulated_harvesting_masks.at(*device_id));

            } else if (arch_name == Arch::WORMHOLE_B0 || arch_name == Arch::WORMHOLE) {
                log_assert(
    ...
    perform_harvesting_and_populate_soc_descriptors(m_pci_device_map[0]->get_soc_descriptor(), perform_harvesting);
    populate_cores();
    ...
    if (arch_name == Arch::WORMHOLE or arch_name == Arch::WORMHOLE_B0) {
        ...
        for (const auto& logical_mmio_chip_id : target_mmio_device_ids) {
            SocDescriptor& soc_desc = get_soc_descriptor(logical_mmio_chip_id);
...
void LocalChip::configure_active_ethernet_cores_for_mmio_device(
    ...
    log_assert(
        get_soc_descriptor(mmio_chip).arch == Arch::WORMHOLE_B0,
    ...
    static std::unordered_set<xy_pair> eth_cores_available_if_active = {
        eth_cores.at(0), eth_cores.at(1), eth_cores.at(6), eth_cores.at(7)};
    // Eth cores 8 and 9 are always available
    std::vector<cxy_pair> non_mmio_access_cores_for_chip = {
        cxy_pair(mmio_chip, eth_cores.at(8)), cxy_pair(mmio_chip, eth_cores.at(9))};
    for (const auto& active_eth_core : active_eth_cores_per_chip) {
        if (eth_cores_available_if_active.find(active_eth_core) != eth_cores_available_if_active.end()) {
            non_mmio_access_cores_for_chip.push_back(cxy_pair(mmio_chip, active_eth_core));
        }
    }
...
void LocalChip::populate_cores() {
    std::uint32_t count = 0;
    for (const auto chip : soc_descriptor_per_chip) {
        workers_per_chip.insert(
            {chip.first, std::unordered_set<xy_pair>(chip.second.workers.begin(), chip.second.workers.end())});
        if (count == 0) {
            eth_cores =
                std::unordered_set<xy_pair>(chip.second.ethernet_cores.begin(), chip.second.ethernet_cores.end());
...
std::vector<int> LocalChip::extract_rows_to_remove(
    ...
    if (arch == Arch::WORMHOLE || arch == Arch::WORMHOLE_B0) {
...
void LocalChip::remove_worker_row_from_descriptor(
    SocDescriptor& full_soc_descriptor, const std::vector<int>& row_coordinates_to_remove) {
    std::vector<xy_pair> workers_to_keep;
...
void LocalChip::harvest_rows_in_soc_descriptor(Arch arch, SocDescriptor& sdesc, uint32_t harvested_rows) {
...
void LocalChip::perform_harvesting_and_populate_soc_descriptors(
    const SocDescriptor& soc_descriptor, const bool perform_harvesting) {
    for (const auto& chip : harvested_rows_per_target) {
        auto temp_sdesc = soc_descriptor;
...
void LocalChip::check_pcie_device_initialized(int device_id) {

    // brosko: see if there's a way to confirm arch is as expected?
    TTDevice* tt_device = get_pci_device(device_id);
    // if (arch_name == Arch::GRAYSKULL) {
    //     if (!is_grayskull(pci_device->device_id)) {
    //         throw std::runtime_error(
    //             "Attempted to run grayskull configured tt_device on " + get_arch_str(detect_arch(pci_device)));
    //     }
    // } else if (arch_name == Arch::WORMHOLE || arch_name == Arch::WORMHOLE_B0) {
    //     if (!is_wormhole(pci_device->device_id)) {
    //         throw std::runtime_error(
    //             "Attempted to run wormhole configured tt_device on " + get_arch_str(detect_arch(pci_device)));
    //     }
    // } else if (arch_name == Arch::BLACKHOLE) {
    //     if (!is_blackhole(pci_device->device_id)) {
    //         throw std::runtime_error(
    //             "Attempted to run blackhole configured tt_device on " + get_arch_str(detect_arch(pci_device)));
    //     }
    // } else {
    ...
    if (arch_name != Arch::BLACKHOLE) {
        ...
        uint32_t bar_read_initial =
            get_pci_device(device_id)->bar_read32(tt_device->get_arc_reset_scratch_offset() + 3 * 4);
        uint32_t arg = bar_read_initial == 500 ? 325 : 500;
        uint32_t bar_read_again;
        uint32_t arc_msg_return = arc_msg(
            device_id, 0xaa00 | tt_device->get_arc_message_test(), true, arg, 0, 1, &bar_read_again);
        if (arc_msg_return != 0 || bar_read_again != arg + 1) {
            auto postcode = get_pci_device(device_id)->bar_read32(tt_device->get_arc_reset_scratch_offset());
            throw std::runtime_error(
                "Device is not initialized: arc_fw postcode: " + std::to_string(postcode) +
                " arc_msg_return: " + std::to_string(arc_msg_return) + " arg: " + std::to_string(arg) +
...
std::unordered_map<xy_pair, xy_pair> LocalChip::create_harvested_coord_translation(
    const Arch arch, bool identity_map) {
    log_assert(
        identity_map ? true : (arch != Arch::GRAYSKULL), "NOC Translation can only be performed for WH devices");
    std::unordered_map<xy_pair, xy_pair> translation_table = {};

    xy_pair grid_size;
    std::vector<uint32_t> T6_x = {};
    std::vector<uint32_t> T6_y = {};
    std::vector<xy_pair> ethernet = {};
    // Store device specific data for GS and WH depending on arch
    if (arch == Arch::GRAYSKULL) {
        grid_size = xy_pair(13, 12);
        T6_x = {12, 1, 11, 2, 10, 3, 9, 4, 8, 5, 7, 6};
        T6_y = {11, 1, 10, 2, 9, 3, 8, 4, 7, 5};
    } else if (arch == Arch::BLACKHOLE) {
        grid_size = xy_pair(17, 12);
        T6_x = {16, 1, 15, 2, 14, 3, 13, 4, 12, 5, 11, 6, 10, 7};
        T6_y = {11, 2, 10, 3, 9, 4, 8, 5, 7, 6};
    } else {
        grid_size = xy_pair(10, 12);
...
void LocalChip::translate_to_noc_table_coords(chip_id_t device_id, std::size_t& r, std::size_t& c) {
    auto translated_coords = harvested_coord_translation[device_id].at(xy_pair(c, r));
    c = translated_coords.x;
    r = translated_coords.y;
}

void LocalChip::initialize_pcie_devices() {
    LOG1("---- LocalChip::start\n");
    ...
    // TODO: Implement support for multiple host channels on BLACKHOLE.
    log_assert(
        !(arch_name == Arch::BLACKHOLE && m_num_host_mem_channels > 1),
...
void LocalChip::broadcast_pcie_tensix_risc_reset(TTDevice* device, const TensixSoftResetOptions& soft_resets) {
    LOG1("---- LocalChip::broadcast_tensix_risc_reset\n");
    ...
    auto [soft_reset_reg, _] = device->set_dynamic_tlb_broadcast(
        device->get_reg_tlb(),
        device->get_tensix_soft_reset_addr(),
        harvested_coord_translation,
        xy_pair(0, 0),
        xy_pair(
            device->get_grid_size_x() - 1,
            device->get_grid_size_y() - 1 - num_rows_harvested.at(device->pci_device->logical_id)),
        tlb_data::Posted);
    device->pci_device->write_regs(soft_reset_reg, 1, &valid);
    tt_driver_atomics::sfence();
}

std::set<chip_id_t> LocalChip::get_target_mmio_device_ids() {
...
void LocalChip::assert_risc_reset() { broadcast_tensix_risc_reset_to_cluster(TENSIX_ASSERT_SOFT_RESET); }

void LocalChip::deassert_risc_reset() { broadcast_tensix_risc_reset_to_cluster(TENSIX_DEASSERT_SOFT_RESET); }

void LocalChip::deassert_risc_reset_at_core(cxy_pair core) {
    ...
    if (target_is_mmio_capable) {
        ...
        log_assert(arch_name != Arch::BLACKHOLE, "Can't issue access to remote core in BH");
        send_remote_tensix_risc_reset_to_core(core, TENSIX_DEASSERT_SOFT_RESET);
    }
}

void LocalChip::assert_risc_reset_at_core(cxy_pair core) {
    std::uint32_t target_device =
        core.chip;  // Get Target Device to query soc descriptor and determine location in cluster
    log_assert(
        std::find(
...
void LocalChip::cleanup_shared_host_state() {
...
std::unordered_set<chip_id_t> LocalChip::get_all_chips_in_cluster() { return ndesc->get_all_chips(); }
int LocalChip::get_number_of_chips_in_cluster() {
    // Returns the number of chips seen in the network descriptor
    return ndesc->get_all_chips().size();
}

ClusterDescriptor* LocalChip::get_cluster_description() { return ndesc.get(); }
// Can be used before instantiating a silicon device
int LocalChip::detect_number_of_chips() {
    auto available_device_ids = detect_available_device_ids();
    return available_device_ids.size();
}

// Can be used before instantiating a silicon device
std::vector<chip_id_t> LocalChip::detect_available_device_ids() {
...
// brosko: remove auto everywhere
Writer LocalChip::get_static_tlb_writer(cxy_pair target) {
    ...
    auto tlb_index = map_core_to_tlb(xy_pair(target.x, target.y));
    auto tlb_data = dev->describe_tlb(tlb_index);
    ...
    uint8_t* base = reinterpret_cast<uint8_t*>(dev->pci_device->bar0_wc);

    return Writer(base + tlb_offset, tlb_size);
}

void LocalChip::write_device_memory(
    const void* mem_ptr,
    ...
    // LOG1("---- LocalChip::write_device_memory to chip:%lu %lu-%lu at 0x%x size_in_bytes: %d small_access: %d\n",
    ...
    if (tlbs_init) {
        tlb_index = map_core_to_tlb(xy_pair(target.x, target.y));
        tlb_data = dev->describe_tlb(tlb_index);
    }
    ...
    if (tlb_data.has_value() &&
        ...
        dev->write_block_through_tlb(tlb_offset, address, tlb_size, size_in_bytes, buffer_addr);
    } else {
        const auto tlb_index = dynamic_tlb_config.at(fallback_tlb);
        // brosko: so at many places pci_device->id is used
        // figure out logical device id physical device which is which and remove redundant id
        const scoped_lock<named_mutex> lock(*get_mutex(fallback_tlb, dev->pci_device->logical_id));
...
void LocalChip::read_device_memory(
    void* mem_ptr,
    cxy_pair target,
    std::uint32_t address,
    ...
    LOG1(
        "---- LocalChip::read_device_memory to chip:%lu %lu-%lu at 0x%x size_in_bytes: %d\n",
        target.chip,
        target.x,
        target.y,
        address,
    ...
    if (tlb_data.has_value() &&
        ...
        dev->read_block_through_tlb(tlb_offset, address, tlb_size, size_in_bytes, buffer_addr);
...
void LocalChip::read_buffer(
    ...
    if (hugepage_mapping.at(src_device_id).at(channel)) {
        ...
        std::string err_msg =
            "write_buffer: Hugepages are not allocated for src_device_id: " + std::to_string(src_device_id) +
    ...
    LOG1(
        "---- LocalChip::read_buffer (src_device_id: %d, ch: %d) from 0x%lx\n",
        src_device_id,
        channel,
        user_scratchspace);
...
void LocalChip::write_buffer(
    ...
    if (hugepage_mapping.at(src_device_id).at(channel)) {
        ...
        std::string err_msg =
            "write_buffer: Hugepage are not allocated for src_device_id: " + std::to_string(src_device_id) +
            " ch: " + std::to_string(channel);
        throw std::runtime_error(err_msg);
    }
...
uint32_t LocalChip::get_power_state_arc_msg(TTDevice* tt_device, DevicePowerState state) {
    uint32_t msg = 0xaa00;
    switch (state) {
        case BUSY: {
            msg |= tt_device->get_arc_message_arc_go_busy();
            break;
        }
        case LONG_IDLE: {
            msg |= tt_device->get_arc_message_arc_go_long_idle();
            break;
        }
        case SHORT_IDLE: {
            msg |= tt_device->get_arc_message_arc_go_short_idle();
            break;
        }
...
void LocalChip::set_pcie_power_state(DevicePowerState state) {
    for (auto& device_it : m_pci_device_map) {
        int d = device_it.first;
        TTDevice* tt_device = device_it.second.get();
        uint32_t msg = get_power_state_arc_msg(tt_device, state);
        std::stringstream ss;
        ss << state;
...
int LocalChip::get_clock(int logical_device_id) {
    ...
    TTDevice* tt_device = get_pci_device(mmio_capable_chip_logical);
    auto exit_code = arc_msg(
        logical_device_id,
        0xaa00 | tt_device->get_arc_message_get_aiclk(),
        true,
        0xFFFF,
        0xFFFF,
        1,
...
std::map<int, int> LocalChip::get_clocks() {
...
LocalChip::~LocalChip() {
    LOG1("---- LocalChip::~LocalChip\n");

    for (int i = 0; i < archs_in_cluster.size(); i++) {
        if (archs_in_cluster[i] == Arch::WORMHOLE) {
            log_warning(
                LogSiliconDriver,
                "Virtual device {} for this run is Wormhole A0. This architecture is now deprecated. Please use "
                "Wormhole B0 for testing.",
                i);
        }
    }
...
std::optional<std::tuple<uint32_t, uint32_t>> LocalChip::get_tlb_data_from_target(const xy_pair& target) {
    std::int32_t tlb_index = 0;
    std::optional<std::tuple<std::uint32_t, std::uint32_t>> tlb_data;

    if (tlbs_init) {
        tlb_index = map_core_to_tlb(target);
        // brosko: well doesn't make sense for multi chip, right? I've added this for getting first pcie, might be erroneous
...
void LocalChip::configure_tlb(
    chip_id_t logical_device_id, xy_pair core, std::int32_t tlb_index, std::int32_t address, uint64_t ordering) {
    log_assert(
        ordering == tlb_data::Strict || ordering == tlb_data::Posted || ordering == tlb_data::Relaxed,
        "Invalid ordering specified in LocalChip::configure_tlb");
    TTDevice* tt_device = get_pci_device(logical_device_id);
    tt_device->set_dynamic_tlb(tlb_index, core, address, harvested_coord_translation, ordering);
    auto tlb_size = std::get<1>(tt_device->describe_tlb(tlb_index).value());
...
void LocalChip::set_fallback_tlb_ordering_mode(const std::string& fallback_tlb, uint64_t ordering) {
    log_assert(
        ordering == tlb_data::Strict || ordering == tlb_data::Posted || ordering == tlb_data::Relaxed,
        "Invalid ordering specified in LocalChip::configure_tlb.");
    log_assert(
        dynamic_tlb_ordering_modes.find(fallback_tlb) != dynamic_tlb_ordering_modes.end(),
        "Invalid TLB specified in LocalChip::set_fallback_tlb_ordering_mode.");
    log_assert(
...
void LocalChip::init_pcie_iatus() {
    ...
    LOG1("---- LocalChip::init_pcie_iatus() num_enabled_devices: %d\n", num_enabled_devices);
    log_assert(
    ...
    for (auto& src_device_it : m_pci_device_map) {
        ...
        TTDevice* src_tt_device = src_device_it.second.get();
        ...
        for (int channel_id = 0; channel_id < m_num_host_mem_channels; channel_id++) {
            if (hugepage_mapping.at(src_pci_id).at(channel_id)) {
                ...
                log_debug(
                    LogSiliconDriver, "Configuring ATU channel {} to point to hugepage {}.", channel_id, src_pci_id);
                iatu_configure_peer_region(
                    src_pci_id, channel_id, hugepage_physical_address.at(src_pci_id).at(channel_id), region_size);
                if (host_channel_size.find(src_tt_device->pci_device->logical_id) == host_channel_size.end()) {
                    host_channel_size.insert({(int)src_tt_device->pci_device->logical_id, {}});
                }
                host_channel_size.at(src_tt_device->pci_device->logical_id).push_back(region_size);
            } else {
                std::string err_msg =
                    "init_pcie_iatus: Hugepages are not allocated for src_pci_id: " + std::to_string(src_pci_id) +
...
int LocalChip::open_hugepage_file(const std::string& dir, chip_id_t physical_device_id, uint16_t channel) {
    ...
    LOG1(
        "---- ttSiliconDevice::open_hugepage_file: using filename: %s for physical_device_id: %d channel: %d\n",
        filename_str.c_str(),
        physical_device_id,
        channel);
    ...
    if (fd == -1 && errno == EACCES) {
        WARN(
            "---- ttSiliconDevice::open_hugepage_file could not open filename: %s on first try, unlinking it and "
            "retrying.\n",
            filename_str.c_str());
...
bool LocalChip::init_hugepage(chip_id_t device_id) {
    ...
    // brosko: yep, check this logic
    auto physical_device_id = m_pci_device_map.at(device_id)->pci_device->logical_id;
    ...
    for (int ch = 0; ch < m_num_host_mem_channels; ch++) {
        ...
        if (hugepage_fd == -1) {
            ...
            WARN(
                "---- ttSiliconDevice::init_hugepage: physical_device_id: %d ch: %d creating hugepage mapping file "
                "failed.\n",
                physical_device_id,
                ch);
            success = false;
        ...
        std::byte* mapping = static_cast<std::byte*>(
            mmap(nullptr, mapping_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, hugepage_fd, 0));

        close(hugepage_fd);

        if (mapping == MAP_FAILED) {
            uint32_t num_tt_mmio_devices_for_arch =
                cpuset::tt_cpuset_allocator::get_num_tt_pci_devices_by_pci_device_id(
                    m_pci_device_map.at(device_id)->device_id, m_pci_device_map.at(device_id)->pci_device->get_revision_id());
            WARN(
                "---- ttSiliconDevice::init_hugepage: physical_device_id: %d ch: %d mapping hugepage failed. (errno: "
                "%s).\n",
                physical_device_id,
                ch,
                strerror(errno));
            WARN(
                "---- Possible hint: /proc/cmdline should have hugepages=N, nr_hugepages=N - (N = NUM_MMIO_TT_DEVICES "
                "* (is_grayskull ? 1 : 4). NUM_MMIO_DEVICES = %d\n",
        ...
        if (!cpuset::tt_cpuset_allocator::bind_area_to_memory_nodeset(physical_device_id, mapping, mapping_size)) {
            WARN(
                "---- ttSiliconDevice::init_hugepage: bind_area_to_memory_nodeset() failed (physical_device_id: %d ch: "
                "%d). "
        ...
        auto& fd = g_SINGLE_PIN_PAGE_PER_FD_WORKAROND ? m_pci_device_map.at(device_id)->pci_device->device_fd_per_host_ch[ch]
                                                      : m_pci_device_map.at(device_id)->pci_device->device_fd;

        if (ioctl(fd, TENSTORRENT_IOCTL_PIN_PAGES, &pin_pages) == -1) {
            WARN(
                "---- ttSiliconDevice::init_hugepage: physical_device_id: %d ch: %d TENSTORRENT_IOCTL_PIN_PAGES failed "
                "(errno: %s). Common Issue: Requires TTMKD >= 1.11, see following file contents...\n",
                physical_device_id,
                ch,
                strerror(errno));
        ...
        LOG1(
            "---- ttSiliconDevice::init_hugepage: physical_device_id: %d ch: %d mapping_size: %d physical address "
            "0x%llx\n",
            physical_device_id,
            ch,
            mapping_size,
...
int LocalChip::test_setup_interface() {
    // brosko: remove arch_name from LocalChip
    // brosko: remove all if(arch_name) statements and push them to TTDevice
    int ret_val = 0;
    std::unique_ptr<TTDevice>& dev = m_pci_device_map.begin()->second;
    if (arch_name == Arch::GRAYSKULL) {
        ...
        uint32_t regval = 0;
        dev->pci_device->read_regs(mapped_reg, 1, &regval);
        ...
        // uint32_t mapped_reg = set_dynamic_tlb(m_pci_device_map.begin()->second,
        // dev->get_architecture_implementation()->get_reg_tlb(), xy_pair(1, 0), 0xffb20108,
...
int LocalChip::iatu_configure_peer_region(
    ...
    tt_device->program_atu(base_addr, base_size, bar_addr_64);

    // Print what just happened
    uint32_t peer_region_start = region_id_to_use * region_size;
    uint32_t peer_region_end = (region_id_to_use + 1) * region_size - 1;
    LOG1(
        "    [region id %d] NOC to PCI address range 0x%x-0x%x mapped to addr 0x%llx\n",
        peer_region_id,
        peer_region_start,
        peer_region_end,
        bar_addr_64);
...
uint32_t LocalChip::get_harvested_noc_rows(uint32_t harvesting_mask) {
    // auto architecture_implementation =
    //     tt::umd::architecture_implementation::create(static_cast<tt::umd::architecture>(arch_name));
    const std::vector<uint32_t>& harv_to_noc_loc = m_pci_device_map.begin()->second->get_harvesting_noc_locations();
...
uint32_t LocalChip::get_harvested_rows(int logical_device_id) {
    ...
    if (harv_override) {
        ...
        int harvesting_msg_code = arc_msg(
            logical_device_id,
            0xaa00 | tt_device->get_arc_message_arc_get_harvesting(),
            true,
            0,
            0,
            1,
...
uint32_t LocalChip::get_harvested_noc_rows_for_chip(int logical_device_id) {
    return get_harvested_noc_rows(get_harvested_rows(logical_device_id));
}

void LocalChip::enable_local_ethernet_queue(const chip_id_t& device_id, int timeout) {
    ...
    while (msg_success != 1) {
        if (std::chrono::system_clock::now() - start > timeout_seconds) {
            throw std::runtime_error(
                "Timed out after waiting " + std::to_string(timeout) + " seconds for DRAM to finish training");
        }
...
void* LocalChip::host_dma_address(std::uint64_t offset, chip_id_t src_device_id, uint16_t channel) const {
...
// brosko create a task to remove all inlines and overrides where unnecessary.
// brosko: change this to return a unique_ptr reference?
// brosko: this should be moved to tt_cluster in anycase
// Wrapper for throwing more helpful exception when not-enabled pci intf is accessed.
TTDevice* LocalChip::get_pci_device(int device_id) const {
    if (!m_pci_device_map.count(device_id)) {
        throw std::runtime_error(
            std::string("device_id: " + std::to_string(device_id) + " attempted to be accessed, but is not enabled."));
    }
    return m_pci_device_map.at(device_id).get();
}

std::shared_ptr<boost::interprocess::named_mutex> LocalChip::get_mutex(
...
uint64_t LocalChip::get_sys_addr(uint32_t chip_x, uint32_t chip_y, uint32_t noc_x, uint32_t noc_y, uint64_t offset) {
...
uint16_t LocalChip::get_sys_rack(uint32_t rack_x, uint32_t rack_y) {
...
bool LocalChip::is_non_mmio_cmd_q_full(uint32_t curr_wptr, uint32_t curr_rptr) {
...
void LocalChip::write_to_non_mmio_device(
    const void* mem_ptr,
    uint32_t size_in_bytes,
    cxy_pair core,
    uint64_t address,
    ...
    const scoped_lock<named_mutex> lock(
        *get_mutex(NON_MMIO_MUTEX_NAME, this->get_pci_device(mmio_capable_chip_logical)->device_id));
    ...
    cxy_pair remote_transfer_ethernet_core =
 ...
 */
void LocalChip::read_from_non_mmio_device(void* mem_ptr, cxy_pair core, uint64_t address, uint32_t size_in_bytes) {
    using data_word_t = uint32_t;
    constexpr int DATA_WORD_SIZE = sizeof(data_word_t);
    std::string write_tlb = "LARGE_WRITE_TLB";
    std::string read_tlb = "LARGE_READ_TLB";
    ...
    const scoped_lock<named_mutex> lock(
        *get_mutex(NON_MMIO_MUTEX_NAME, this->get_pci_device(mmio_capable_chip_logical)->device_id));
    const cxy_pair remote_transfer_ethernet_core = remote_transfer_ethernet_cores[mmio_capable_chip_logical].at(0);
    ...
    while (offset < size_in_bytes) {
        ...
        if (erisc_resp_flags[0] == resp_flags) {
            ...
            if (block_size == DATA_WORD_SIZE) {
                ...
                if (use_dram) {
                    read_from_sysmem(
                        data_block, host_dram_block_addr, host_dram_channel, block_size, mmio_capable_chip_logical);
                } else {
...
void LocalChip::wait_for_non_mmio_flush() {
    if (flush_non_mmio) {
        log_assert(arch_name != Arch::BLACKHOLE, "Non-MMIO flush not supported in Blackhole");
        std::string read_tlb = "LARGE_READ_TLB";
        ...
        for (auto chip_id : chips_with_mmio) {
            ...
            if (arch == Arch::WORMHOLE || arch == Arch::WORMHOLE_B0) {
                ...
                for (cxy_pair& cxy : remote_transfer_ethernet_cores.at(chip_id)) {
                    do {
                        read_device_memory(
                            erisc_q_ptrs.data(),
                            cxy,
...
// Broadcast Functions
void LocalChip::generate_tensix_broadcast_grids_for_grayskull(
    ...
    for (const auto& x_pair : bb_x_coords) {
        for (const auto& y_pair : bb_y_coords) {
            xy_pair top_left = xy_pair(x_pair.first, y_pair.first);
            xy_pair bot_right = xy_pair(x_pair.second, y_pair.second);
            broadcast_grids.insert({top_left, bot_right});
        }
    }
}

std::unordered_map<chip_id_t, std::vector<std::vector<int>>>& LocalChip::get_ethernet_broadcast_headers(
...
void LocalChip::pcie_broadcast_write(
    chip_id_t chip,
    const void* mem_ptr,
    uint32_t size_in_bytes,
    std::uint32_t addr,
    ...
    // Broadcast for WH.
    TTDevice* dev = get_pci_device(chip);
    const auto tlb_index = dynamic_tlb_config.at(fallback_tlb);
    const uint8_t* buffer_addr = static_cast<const uint8_t*>(mem_ptr);
    const scoped_lock<named_mutex> lock(*get_mutex(fallback_tlb, dev->pci_device->logical_id));
    while (size_in_bytes > 0) {
        auto [mapped_address, tlb_size] = dev->set_dynamic_tlb_broadcast(
            tlb_index,
            addr,
            harvested_coord_translation,
            start,
    ...
    if (use_ethernet_broadcast) {
        ...
        for (auto& mmio_group : broadcast_headers) {
            for (auto& header : mmio_group.second) {
                ...
                // Write Target: x-y endpoint is a don't care. Initialize to xy_pair(1, 1)
                write_to_non_mmio_device(
                    mem_ptr, size_in_bytes, cxy_pair(mmio_group.first, xy_pair(1, 1)), address, true, header);
            }
        }
    } else {
        // Broadcast not supported. Implement this at the software level as a for loop
        std::vector<cxy_pair> cores_to_write = {};
        for (const auto& chip : target_devices_in_cluster) {
            if (chips_to_exclude.find(chip) != chips_to_exclude.end())
                continue;
            for (const auto& core : get_soc_descriptor(chip).cores) {
                if (cols_to_exclude.find(core.first.x) == cols_to_exclude.end() and
                    ...
                    write_to_device(
                        mem_ptr, size_in_bytes, cxy_pair(chip, core.first.x, core.first.y), address, fallback_tlb);
                }
            }
        }
    }
...
void LocalChip::broadcast_write_to_cluster(
    const void* mem_ptr,
    uint32_t size_in_bytes,
    uint64_t address,
    const std::set<chip_id_t>& chips_to_exclude,
    ...
    if (arch_name == Arch::GRAYSKULL) {
        ...
        std::vector<xy_pair> dram_cores_to_write = {};
        ...
        for (const auto& row : dram_rows) {
            for (const auto& col : dram_cols) {
                if (rows_to_exclude.find(row) == rows_to_exclude.end() and
                    ...
                    dram_cores_to_write.push_back(xy_pair(col, row));
                }
            }
        }
        ...
        for (const auto& chip : target_devices_in_cluster) {
            ...
            for (const auto& dram : dram_cores_to_write) {
                write_device_memory(mem_ptr, size_in_bytes, cxy_pair(chip, dram), address, fallback_tlb);
            }
        ...
        if (cols_to_exclude.find(0) == cols_to_exclude.end() or cols_to_exclude.find(9) == cols_to_exclude.end()) {
            log_assert(
                !dev->tensix_or_eth_in_broadcast(cols_to_exclude),
            ...
            log_assert(
                use_virtual_coords_for_eth_broadcast or
                    dev->valid_tensix_broadcast_grid(rows_to_exclude, cols_to_exclude),
...
int LocalChip::remote_arc_msg(
    ...
    auto core = cxy_pair(chip, get_soc_descriptor(chip).arc_cores.at(0));
        ...
        while (true) {
            if (std::chrono::system_clock::now() - start > timeout_seconds) {
                ...
                throw std::runtime_error(
                    ...
                    std::to_string(chip) + " ARC to respond to message 0x" + ss.str());
            }
...
void LocalChip::write_to_sysmem(
...
void LocalChip::write_to_sysmem(std::vector<uint32_t>& vec, uint64_t addr, uint16_t channel, chip_id_t src_device_id) {
    write_buffer(vec.data(), vec.size() * sizeof(uint32_t), addr, channel, src_device_id);
}

void LocalChip::read_from_sysmem(
...
void LocalChip::set_membar_flag(
    ...
    std::unordered_set<xy_pair> cores_synced = {};
    std::vector<uint32_t> barrier_val_vec = {barrier_value};
    for (const auto& core : cores) {
        write_to_device(barrier_val_vec, cxy_pair(chip, core), barrier_addr, fallback_tlb);
    }
    ...
    while (cores_synced.size() != cores.size()) {
        for (const auto& core : cores) {
            if (cores_synced.find(core) == cores_synced.end()) {
                ...
                read_from_device(
                    readback_vec, cxy_pair(chip, core), barrier_addr, sizeof(std::uint32_t), fallback_tlb);
...
void LocalChip::insert_host_to_device_barrier(
    const chip_id_t chip,
    const std::unordered_set<xy_pair>& cores,
    const uint32_t barrier_addr,
    const std::string& fallback_tlb) {
...
void LocalChip::l1_membar(
    ...
    if (ndesc->is_chip_mmio_capable(chip)) {
        ...
        if (cores.size()) {
            ...
            std::unordered_set<xy_pair> workers_to_sync = {};
            std::unordered_set<xy_pair> eth_to_sync = {};
...
void LocalChip::dram_membar(
    ...
    if (ndesc->is_chip_mmio_capable(chip)) {
        if (channels.size()) {
            std::unordered_set<xy_pair> dram_cores_to_sync = {};
...
void LocalChip::write_to_device(
    const void* mem_ptr, uint32_t size, cxy_pair core, uint64_t addr, const std::string& fallback_tlb) {
    ...
    if (target_is_mmio_capable) {
        ...
        log_assert(arch_name != Arch::BLACKHOLE, "Non-MMIO targets not supported in Blackhole");
        log_assert(
...
void LocalChip::write_to_device(
    std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, const std::string& fallback_tlb) {
    // Overloaded device writer that accepts a vector
    write_to_device(vec.data(), vec.size() * sizeof(uint32_t), core, addr, fallback_tlb);
}

void LocalChip::read_mmio_device_register(
    void* mem_ptr, cxy_pair core, uint64_t addr, uint32_t size, const std::string& fallback_tlb) {
    TTDevice* tt_device = get_pci_device(core.chip);

    const auto tlb_index = dynamic_tlb_config.at(fallback_tlb);
    const scoped_lock<named_mutex> lock(*get_mutex(fallback_tlb, tt_device->device_id));
    LOG1("  dynamic tlb_index: %d\n", tlb_index);

    auto [mapped_address, tlb_size] =
        tt_device->set_dynamic_tlb(tlb_index, core, addr, harvested_coord_translation, tlb_data::Strict);
    // Align block to 4bytes if needed.
    auto aligned_buf = tt_4_byte_aligned_buffer(mem_ptr, size);
    tt_device->pci_device->read_regs(mapped_address, aligned_buf.block_size / sizeof(std::uint32_t), aligned_buf.local_storage);
...
void LocalChip::write_mmio_device_register(
    ...
    tt_device->pci_device->write_regs(mapped_address, aligned_buf.block_size / sizeof(uint32_t), aligned_buf.local_storage);
}
...
void LocalChip::read_from_device(
    std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, uint32_t size, const std::string& fallback_tlb) {
    size_buffer_to_capacity(vec, size);
...
int LocalChip::arc_msg(
    ...
    uint32_t* return_3,
    uint32_t* return_4) {
    log_assert(arch_name != Arch::BLACKHOLE, "ARC messages not supported in Blackhole");
    if (ndesc->is_chip_mmio_capable(logical_device_id)) {
        // Exclusive access for a single process at a time. Based on physical pci interface id.
        std::string msg_type = "ARC_MSG";
        const scoped_lock<named_mutex> lock(*get_mutex(msg_type, logical_device_id));
        return get_pci_device(logical_device_id)->pcie_arc_msg(msg_code, wait_for_done, arg0, arg1, timeout, return_3, return_4);
    } else {
        return remote_arc_msg(logical_device_id, msg_code, wait_for_done, arg0, arg1, timeout, return_3, return_4);
    }
}

void LocalChip::send_tensix_risc_reset_to_core(const cxy_pair& core, const TensixSoftResetOptions& soft_resets) {
    auto valid = soft_resets & ALL_TENSIX_SOFT_RESET;
...
void LocalChip::send_remote_tensix_risc_reset_to_core(
    const cxy_pair& core, const TensixSoftResetOptions& soft_resets) {
    auto valid = soft_resets & ALL_TENSIX_SOFT_RESET;
    uint32_t valid_val = (std::underlying_type<TensixSoftResetOptions>::type)valid;
...
int LocalChip::set_remote_power_state(const chip_id_t& chip, DevicePowerState device_state) {
    auto mmio_capable_chip_logical = ndesc->get_closest_mmio_capable_chip(chip);
    TTDevice* tt_device = get_pci_device(mmio_capable_chip_logical);
    return remote_arc_msg(chip, get_power_state_arc_msg(tt_device, device_state), true, 0, 0, 1, NULL, NULL);
}

void LocalChip::enable_remote_ethernet_queue(const chip_id_t& chip, int timeout) {
    uint32_t msg_success = 0x0;
    auto timeout_seconds = std::chrono::seconds(timeout);
    auto start = std::chrono::system_clock::now();
    while (msg_success != 1) {
...
void LocalChip::broadcast_tensix_risc_reset_to_cluster(const TensixSoftResetOptions& soft_resets) {
    if (arch_name == Arch::GRAYSKULL) {
        for (auto& device_it : m_pci_device_map) {
...
void LocalChip::set_power_state(DevicePowerState device_state) {
...
void LocalChip::enable_ethernet_queue(int timeout) {
    for (const chip_id_t& chip : target_devices_in_cluster) {
        ...
        switch (arch) {
            ...
            case Arch::WORMHOLE_B0: {
                ...
                break;
                case Arch::BLACKHOLE: log_assert(false, "Arch BLACKHOLE doesn't support ethernet queues yet");
            }
            default: {
                break;
            }
...
std::set<chip_id_t> LocalChip::get_target_remote_device_ids() { return target_remote_chips; }

void LocalChip::deassert_resets_and_set_power_state() {
    ...
    if (arch_name != Arch::BLACKHOLE) {
        ...
        for (auto& device_it : m_pci_device_map) {
            arc_msg(
                device_it.first,
                0xaa00 |
                    device_it.second->get_arc_message_deassert_riscv_reset(),
                true,
                0,
                0);
        }
            ...
            for (const chip_id_t& chip : target_devices_in_cluster) {
                if (!ndesc->is_chip_mmio_capable(chip)) {
                    ...
                    remote_arc_msg(
                        chip,
                        0xaa00 |
                            tt_device->get_arc_message_deassert_riscv_reset(),
                        true,
                        0x0,
                        0x0,
                        1,
...
void LocalChip::verify_eth_fw() {
    for (const auto& chip : target_devices_in_cluster) {
        std::vector<uint32_t> mem_vector;
        std::vector<uint32_t> fw_versions;
        for (xy_pair& eth_core : get_soc_descriptor(chip).ethernet_cores) {
            read_from_device(
                mem_vector,
                cxy_pair(chip, eth_core),
...
void LocalChip::verify_sw_fw_versions(
...
void LocalChip::start_device(const device_params& device_params) {
    if (device_params.init_device) {
        initialize_pcie_devices();
        // MT Initial BH - Ethernet firmware not present in Blackhole
        if (arch_name == Arch::WORMHOLE || arch_name == Arch::WORMHOLE_B0) {
            verify_eth_fw();
        }
...
void LocalChip::set_device_l1_address_params(const device_l1_address_params& l1_address_params_) {
    l1_address_params = l1_address_params_;
}

void LocalChip::set_device_dram_address_params(const device_dram_address_params& dram_address_params_) {
    dram_address_params = dram_address_params_;
}

void LocalChip::set_driver_host_address_params(const driver_host_address_params& host_address_params_) {
    host_address_params = host_address_params_;
}

void LocalChip::set_driver_eth_interface_params(const driver_eth_interface_params& eth_interface_params_) {
    eth_interface_params = eth_interface_params_;
}

void LocalChip::setup_core_to_tlb_map(std::function<std::int32_t(xy_pair)> mapping_function) {
    map_core_to_tlb = mapping_function;
    tlbs_init = true;
}

std::uint32_t LocalChip::get_num_dram_channels(std::uint32_t device_id) {
    log_assert(
...
std::uint64_t LocalChip::get_dram_channel_size(std::uint32_t device_id, std::uint32_t channel) {
...
std::uint32_t LocalChip::get_num_host_channels(std::uint32_t device_id) {
    log_assert(
        all_target_mmio_devices.find(device_id) != all_target_mmio_devices.end(),
...
std::uint32_t LocalChip::get_host_channel_size(std::uint32_t device_id, std::uint32_t channel) {
...
std::uint32_t LocalChip::get_numa_node_for_pcie_device(std::uint32_t device_id) {
    return get_pci_device(device_id)->pci_device->get_numa_node();
}

std::uint64_t LocalChip::get_pcie_base_addr_from_device() const {
    if (arch_name == Arch::WORMHOLE or arch_name == Arch::WORMHOLE_B0) {
        return 0x800000000;
    } else if (arch_name == Arch::BLACKHOLE) {
...
tt_version LocalChip::get_ethernet_fw_version() const {
    log_assert(
        arch_name == Arch::WORMHOLE or arch_name == Arch::WORMHOLE_B0,
        "Can only get Ethernet FW version for Wormhole architectures.");
    log_assert(
