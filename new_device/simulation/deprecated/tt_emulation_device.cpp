...
void tt_emulation_device::write(cxy_pair core, uint64_t addr, const std::vector<uint8_t>& data) {
...
std::vector<uint8_t> tt_emulation_device::read(cxy_pair core, uint64_t addr, uint32_t size) {
...
void tt_emulation_device::start_device(const device_params& device_params) {
...
void tt_emulation_device::deassert_risc_reset_at_core(cxy_pair core) {
  tt_zebu_wrapper_inst->tensix_reset_deassert(core.x, core.y);
}

void tt_emulation_device::assert_risc_reset_at_core(cxy_pair core) {
...
void tt_emulation_device::rolled_write_to_device(std::vector<uint32_t>& base_vec, uint32_t unroll_count, cxy_pair core, uint64_t base_addr, const std::string& tlb_to_use) {
...
void tt_emulation_device::write_to_device(const void *mem_ptr, uint32_t size, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd, bool last_send_epoch_cmd, bool ordered_with_prev_remote_write) {
...
void tt_emulation_device::write_to_device(std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd, bool last_send_epoch_cmd, bool ordered_with_prev_remote_write) {
...
void tt_emulation_device::l1_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& cores) {
    // Placeholder - implement later - https://yyz-gitlab.local.tenstorrent.com/tenstorrent/open-umd/-/issues/26
}

void tt_emulation_device::dram_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& cores) {
    // Placeholder - implement later - https://yyz-gitlab.local.tenstorrent.com/tenstorrent/open-umd/-/issues/26
}
...
void tt_emulation_device::read_from_device(std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, uint32_t size, const std::string& /*tlb_to_use*/) {
...
ClusterDescriptor* tt_emulation_device::get_cluster_description() { return ndesc.get(); }
...
void tt_emulation_device::set_device_dram_address_params(const device_dram_address_params& dram_address_params_) {
...
std::unordered_map<chip_id_t, SocDescriptor>& tt_emulation_device::get_virtual_soc_descriptors() {return soc_descriptor_per_chip;}

std::map<int, int> tt_emulation_device::get_clocks() {
  return std::map<int, int>();
}

void tt_emulation_device::set_device_l1_address_params(const device_l1_address_params& l1_address_params_) {
