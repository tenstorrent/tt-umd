...
public:
  virtual void set_device_l1_address_params(const device_l1_address_params& l1_address_params_); // Dont care
  ...
  virtual void write_to_device(std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd = false, bool last_send_epoch_cmd = true, bool ordered_with_prev_remote_write = false);
  ...
  virtual void rolled_write_to_device(std::vector<uint32_t>& base_vec, uint32_t unroll_count, cxy_pair core, uint64_t base_addr, const std::string& tlb_to_use); // See Versim Implementation
  virtual void read_from_device(std::vector<uint32_t>& vec, cxy_pair core, uint64_t addr, uint32_t size, const std::string& tlb_to_use);
...
private:
  ...
  void write(cxy_pair core, uint64_t addr, const std::vector<uint8_t>& data);
  std::vector<uint8_t> read(cxy_pair core, uint64_t addr, uint32_t size);
