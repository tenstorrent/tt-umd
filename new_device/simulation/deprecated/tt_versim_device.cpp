...
#include "chip.h"
#include "new_device/driver_atomics.h"
...
void translate_soc_descriptor_to_ca_soc(CA::Soc &soc, const SocDescriptor soc_descriptor) {
...
tt_VersimDevice::tt_VersimDevice(const std::string &sdesc_path, const std::string &ndesc_path) : tt_device(sdesc_path) {
  soc_descriptor_per_chip.emplace(0, SocDescriptor(sdesc_path));
  std::set<chip_id_t> target_devices = {0};
  if (ndesc_path == "") {
    ndesc = ClusterDescriptor::create_for_grayskull_cluster(target_devices, {});
  } 
  else {
    ndesc = ClusterDescriptor::create_from_yaml(ndesc_path);
  }
}

std::unordered_map<chip_id_t, SocDescriptor>& tt_VersimDevice::get_virtual_soc_descriptors() {return soc_descriptor_per_chip;}

ClusterDescriptor* tt_VersimDevice::get_cluster_description() {return ndesc.get();}
void tt_VersimDevice::start_device(const device_params &device_params) {
...
void tt_VersimDevice::deassert_risc_reset_at_core(cxy_pair core) {
...
void tt_VersimDevice::assert_risc_reset_at_core(cxy_pair core) {
  // This function asserts reset on the full versim device (don't need core level granularity for versim)
 assert_risc_reset();
}

void tt_VersimDevice::rolled_write_to_device(std::vector<uint32_t> &vec, uint32_t unroll_count, cxy_pair core, uint64_t addr, const std::string& tlb_to_use) {
...
void tt_VersimDevice::rolled_write_to_device(uint32_t* mem_ptr, uint32_t len, uint32_t unroll_count, cxy_pair core, uint64_t addr, const std::string& fallback_tlb) {
...
void tt_VersimDevice::write_to_device(std::vector<uint32_t> &vec, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd, bool last_send_epoch_cmd, bool ordered_with_prev_remote_write) {
...
void tt_VersimDevice::write_to_device(const void *mem_ptr, uint32_t size, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd, bool last_send_epoch_cmd, bool ordered_with_prev_remote_write) {
...
void tt_VersimDevice::broadcast_write_to_cluster(const void *mem_ptr, uint32_t size_in_bytes, uint64_t address, const std::set<chip_id_t>& chips_to_exclude, std::set<uint32_t>& rows_to_exclude, std::set<uint32_t>& cols_to_exclude, const std::string& fallback_tlb) {
  for(const auto& core : get_soc_descriptor(0) -> cores) {
    if(cols_to_exclude.find(core.first.x) == cols_to_exclude.end() and rows_to_exclude.find(core.first.y) == rows_to_exclude.end() and core.second.type != CoreType::HARVESTED) {
        write_to_device(mem_ptr, size_in_bytes, cxy_pair(0, core.first.x, core.first.y), address, "");
      }
  }
}
...
void tt_VersimDevice::l1_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& cores) {
...
void tt_VersimDevice::dram_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& dram_cores) {
  tt_driver_atomics::mfence(); // Ensure no reordering of loads/stores around this
}

void tt_VersimDevice::read_from_device(std::vector<uint32_t> &vec, cxy_pair core, uint64_t addr, uint32_t size, const std::string& tlb_to_use) {
...
void tt_VersimDevice::read_from_device(void *mem_ptr, cxy_pair core, uint64_t addr, uint32_t size, const std::string& tlb_to_use) {
  log_debug(tt::LogSiliconDriver, "Versim Device ({}): Read vector from address: {}, with size: {} Bytes", get_sim_time(*versim), addr, size);
...
bool versim_check_dram_core_exists(const std::vector<std::vector<xy_pair>> &dram_core_channels, xy_pair target_core) {
...
void tt_VersimDevice::set_device_l1_address_params(const device_l1_address_params& l1_address_params_) {
    l1_address_params = l1_address_params_;
}

void tt_VersimDevice::set_device_dram_address_params(const device_dram_address_params& dram_address_params_) {
