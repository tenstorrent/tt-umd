...
std::unordered_map<chip_id_t, SocDescriptor>& tt_VersimDevice::get_virtual_soc_descriptors() {
    throw std::runtime_error("tt_VersimDevice() -- VERSIM is not supported in this build\n");
...
void tt_VersimDevice::write_to_device(const void *mem_ptr, uint32_t len, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, bool send_epoch_cmd, bool last_send_epoch_cmd, bool ordered_with_prev_remote_write) {}
...
bool versim_check_dram_core_exists(
    const std::vector<std::vector<xy_pair>> &dram_core_channels, xy_pair target_core) {
  return false;
}
