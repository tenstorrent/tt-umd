...
tt_SimulationDevice::tt_SimulationDevice(const std::string &sdesc_path) : Chip(sdesc_path){
...
std::unordered_map<chip_id_t, SocDescriptor>& tt_SimulationDevice::get_virtual_soc_descriptors() {
    return soc_descriptor_per_chip;
}

void tt_SimulationDevice::set_device_l1_address_params(const device_l1_address_params& l1_address_params_) {
    l1_address_params = l1_address_params_;
}

void tt_SimulationDevice::set_device_dram_address_params(const device_dram_address_params& dram_address_params_) {
    dram_address_params = dram_address_params_;
}

void tt_SimulationDevice::set_driver_host_address_params(const driver_host_address_params& host_address_params_) {
    host_address_params = host_address_params_;
}

void tt_SimulationDevice::set_driver_eth_interface_params(const driver_eth_interface_params& eth_interface_params_) {
    eth_interface_params = eth_interface_params_;
}

void tt_SimulationDevice::start_device(const device_params &device_params) {
...
void tt_SimulationDevice::deassert_risc_reset_at_core(cxy_pair core) {
...
void tt_SimulationDevice::assert_risc_reset_at_core(cxy_pair core) {
...
// Runtime Functions
void tt_SimulationDevice::write_to_device(const void *mem_ptr, uint32_t size_in_bytes, cxy_pair core, uint64_t addr, const std::string& tlb_to_use, 
...
void tt_SimulationDevice::read_from_device(void* mem_ptr, cxy_pair core, uint64_t addr, uint32_t size, const std::string& fallback_tlb) {
    void *rd_resp;
...
void tt_SimulationDevice::l1_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& cores) {}
...
void tt_SimulationDevice::dram_membar(const chip_id_t chip, const std::string& fallback_tlb, const std::unordered_set<xy_pair>& cores) {}
...
flatbuffers::FlatBufferBuilder tt_SimulationDevice::create_flatbuffer(DEVICE_COMMAND rw, std::vector<uint32_t> vec, cxy_pair core_, uint64_t addr, uint64_t size_){
...
void tt_SimulationDevice::print_flatbuffer(const DeviceRequestResponse *buf){    
    ...
    cxy_pair core = {0, buf->core()->x(), buf->core()->y()};
