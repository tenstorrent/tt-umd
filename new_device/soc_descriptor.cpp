...
std::string format_node(xy_pair xy) { return std::to_string(xy.x) + "-" + std::to_string(xy.y); }
...
void SocDescriptor::load_soc_features_from_device_descriptor(YAML::Node &device_descriptor_yaml) {
...
void SocDescriptor::load_core_descriptors_from_device_descriptor(YAML::Node &device_descriptor_yaml) {
    ...
    worker_grid_size = xy_pair(func_x_start, func_y_start);
...
SocDescriptor::SocDescriptor(std::string device_descriptor_path) {
    ...
    grid_size = xy_pair(grid_size_x, grid_size_y);
    physical_grid_size = xy_pair(physical_grid_size_x, physical_grid_size_y);
...
int SocDescriptor::get_num_dram_channels() const {
    int num_channels = 0;
...
bool SocDescriptor::is_worker_core(const xy_pair &core) const {
    return (
        routing_x_to_worker_x.find(core.x) != routing_x_to_worker_x.end() &&
        routing_y_to_worker_y.find(core.y) != routing_y_to_worker_y.end());
}

xy_pair SocDescriptor::get_core_for_dram_channel(int dram_chan, int subchannel) const {
    return this->dram_cores.at(dram_chan).at(subchannel);
};

bool SocDescriptor::is_ethernet_core(const xy_pair &core) const {
    return this->ethernet_core_channel_map.find(core) != ethernet_core_channel_map.end();
}

std::ostream &operator<<(std::ostream &out, const Arch &arch_name) {
    if (arch_name == Arch::JAWBRIDGE) {
        out << "jawbridge";
    } else if (arch_name == Arch::Invalid) {
        out << "none";
