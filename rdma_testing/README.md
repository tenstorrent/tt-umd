...
- `dmabuf_p2p.cpp` — a separate single-binary correctness demo, ported line-for-line from tt-kmd's
  `tools/rdma/nic_bh_p2p_dma.c` onto UMD's `Cluster::export_dmabuf()` (in place of that tool's raw
  `ALLOCATE_TLB`/`CONFIGURE_TLB`/`EXPORT_TLB_DMABUF` ioctls). One binary plays both roles depending
  on its arguments (server: `-c chip_id -r rdma_dev`; client: `-r rdma_dev <server_ip>`) and does a
  full WRITE-then-READ-back round trip with a full-buffer word-by-word verify, chunked/pipelined
  over `OUTSTANDING` in-flight work requests, with the RoCE MTU negotiated from the port instead of
  hardcoded. Use this one to sanity-check the `export_dmabuf()` path against the upstream kmd demo's
  exact flow; use `dmabuf_target`/`dmabuf_initiator` above for bandwidth measurement.
...
```bash
./build/rdma_testing/dmabuf_loopback -r rocep201s0f0
```
...
- Unlike `dmabuf_p2p`, the seed goes in through a *different* TLB window (TlbManager's
  static/cached WC window) than the dedicated one `export_dmabuf()` allocates. So a passing verify
  here is independent evidence that the exported window reaches real device DRAM, rather than the
  self-consistent "write and read back through the same window" check.
