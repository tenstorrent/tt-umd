...
TEST(TestTelemetry, BasicTelemetry) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        uint32_t board_id_high = blackhole_arc_telemetry_reader->read_entry(TelemetryTag::BOARD_ID_HIGH);
        uint32_t board_id_low = blackhole_arc_telemetry_reader->read_entry(TelemetryTag::BOARD_ID_LOW);
...
TEST(TestTelemetry, TelemetryEntryAvailable) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        EXPECT_TRUE(blackhole_arc_telemetry_reader->is_entry_available(TelemetryTag::BOARD_ID_HIGH));
        EXPECT_TRUE(blackhole_arc_telemetry_reader->is_entry_available(TelemetryTag::BOARD_ID_LOW));
