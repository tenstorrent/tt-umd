...
TEST(ApiChipTest, DISABLED_ManualTLBConfiguration) {
    ...
    if (!remote_chips.empty()) {
        ChipId const any_remote_chip = *remote_chips.begin();
        const SocDescriptor& soc_desc = umd_cluster->get_soc_descriptor(any_remote_chip);
        CoreCoord const core = soc_desc.get_cores(CoreType::TENSIX)[0];
    ...
    for (ChipId const mmio_chip : umd_cluster->get_target_mmio_device_ids()) {
        any_mmio_chip = mmio_chip;
        const SocDescriptor& soc_desc = umd_cluster->get_soc_descriptor(mmio_chip);
        for (CoreCoord const core : soc_desc.get_cores(CoreType::TENSIX)) {
    ...
    CoreCoord const dram_core = soc_desc.get_dram_cores()[0][0];
    ...
    if (!eth_cores.empty()) {
        CoreCoord const eth_core = eth_cores[0];
...
TEST(ApiChipTest, SimpleAPIShowcase) {
    ...
    ChipId const chip_id = umd_cluster->get_cluster_description()->get_chips_with_mmio().begin()->first;
