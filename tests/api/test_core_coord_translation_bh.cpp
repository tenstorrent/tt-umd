...
//             CoreCoord physical_coords = coordinate_manager->translate_coord_to(logical_coords,
...
// // For the reverse mapping back of physical to logical coordinates we expect that same logical coordinates are
// returned
...
//                 // Expect that logical to physical translation is 1-1 mapping. No duplicates for physical
//                 coordinates. EXPECT_EQ(physical_coords_set.count(physical_coords), 0);
...
// // For the reverse mapping back of virtual to logical coordinates we expect that same logical coordinates are
// returned
// // as from original mapping.
...
//             for (size_t y = 0; y < tensix_grid_size.y; y++) {
//                 CoreCoord logical_coords = CoreCoord(x, y, CoreType::TENSIX, CoordSystem::LOGICAL);
//                 CoreCoord virtual_coords = coordinate_manager->translate_coord_to(logical_coords,
//                 CoordSystem::VIRTUAL); logical_to_virtual[logical_coords] = virtual_coords;
...
//             CoreCoord logical_coords = coordinate_manager->translate_coord_to(translated_coords,
//             CoordSystem::LOGICAL);
...
//             const CoreCoord dram_physical = coordinate_manager->translate_coord_to(dram_logical,
//             CoordSystem::PHYSICAL);
...
//         CoreCoord virtual_from_physical = coordinate_manager->translate_coord_to(physical_core,
//         CoordSystem::VIRTUAL);
...
//             const CoreCoord pcie_virtual = coordinate_manager->translate_coord_to(pcie_logical,
//             CoordSystem::VIRTUAL); const CoreCoord pcie_physical =
//             coordinate_manager->translate_coord_to(pcie_logical, CoordSystem::PHYSICAL); const tt_xy_pair pcie_core =
//             pcie_cores[y * pcie_grid_size.x + x];
...
//             const CoreCoord arc_physical = coordinate_manager->translate_coord_to(arc_logical,
//             CoordSystem::PHYSICAL); const CoreCoord arc_translated =
...
//         const CoreCoord eth_translated = coordinate_manager->translate_coord_to(eth_logical,
...
//                     CoreCoord(eth_cores[eth_channel].x, eth_cores[eth_channel].y, CoreType::ETH,
//                     CoordSystem::PHYSICAL);
//                 const CoreCoord translated_core =
//                     coordinate_manager->translate_coord_to(physical_core, CoordSystem::TRANSLATED);
