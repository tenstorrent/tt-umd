...
TEST(ApiTTDeviceTest, RuntimeTelemetryBufferAddressAndSize) {
    std::vector<int> pci_device_ids = PCIDevice::enumerate_devices();
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        const auto size = tt_device->get_runtime_telemetry_buffer_size();
        const auto address = tt_device->get_runtime_telemetry_buffer_address();
        ...
        if (size.has_value() && address.has_value()) {
            uint32_t expected_size = 0;
            uint32_t expected_address = 0;
            tt_device->read_from_arc_apb(&expected_size, size_offset.value(), sizeof(expected_size));
            tt_device->read_from_arc_apb(&expected_address, address_offset.value(), sizeof(expected_address));
