...
TEST(ApiTTDeviceTest, TestRemoteTTDevice) {
    ...
    for (chip_id_t remote_chip_id : cluster->get_target_remote_device_ids()) {
        ...
        for (auto channel : cluster_desc->get_active_eth_channels(gateway_id)) {
            active_eth_cores.push_back(closest_local_chip->get_soc_descriptor().get_eth_core_for_channel(channel, CoordSystem::TRANSLATED));
        }
        ...
        std::unique_ptr<RemoteWormholeTTDevice> remote_tt_device = std::make_unique<RemoteWormholeTTDevice>(
            closest_local_chip->get_tt_device(), std::move(remote_communication), remote_eth_coord);
