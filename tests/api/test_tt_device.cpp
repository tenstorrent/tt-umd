...
TEST(ApiTTDeviceTest, TTDeviceRegIO) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        tt_device->write_to_device_reg(tensix_core, data_write0.data(), address, data_write0.size() * sizeof(uint32_t));
        tt_device->read_from_device_reg(tensix_core, data_read.data(), address, data_read.size() * sizeof(uint32_t));
        ASSERT_EQ(data_write0, data_read);
        data_read = std::vector<uint32_t>(data_write0.size(), 0);

        tt_device->write_to_device_reg(tensix_core, data_write1.data(), address, data_write1.size() * sizeof(uint32_t));
        tt_device->read_from_device_reg(tensix_core, data_read.data(), address, data_read.size() * sizeof(uint32_t));
