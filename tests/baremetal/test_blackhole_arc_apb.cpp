...
protected:
    void expect_pcie_x_coordinate(uint32_t x) {
        EXPECT_CALL(pcie_, bar_read32(arch_.get_read_checking_offset())).WillRepeatedly(Return(x));
    }

    uint64_t noc_address() const { return arch_.get_arc_apb_noc_base_address() + APB_OFFSET; }
