...
TEST(GrendelAttMap, MatchesTheTablesTheFirmwareProgrammes) {
    ...
    // Counted so the check runs both ways: walking the snapshot alone would leave a map window
    // unexamined if its mask entry were dropped upstream, and the stale window would still pass on
    // the strength of its endpoint rows.
    size_t matched_windows = 0;
    ...
    for (const auto& entry : tables["mask_table"]) {
        ...
        for (const att::Window& candidate : att::GRENDEL_QSR1_MAP.windows) {
            if (candidate.compare == compare) {
                ASSERT_EQ(window, nullptr) << "two map windows share compare 0x" << std::hex << compare;
                window = &candidate;
            }
        }
    ...
    EXPECT_EQ(matched_windows, att::WINDOW_CLASS_COUNT)
        << "the snapshot no longer carries a mask entry for every window the map declares";
