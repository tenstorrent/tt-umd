...
TEST(BlackholeArcMessages, BlackholeArcMessagesBasic) {
    ...
    for (int const pci_device_id : pci_device_ids) {
        ...
        for (int i = 0; i < num_loops; i++) {
            uint32_t const response = bh_arc_messenger->send_message((uint32_t)blackhole::ArcMessageType::TEST);
...
TEST(BlackholeArcMessages, MultipleThreadsArcMessages) {
    ...
    for (uint32_t const chip_id : cluster->get_target_mmio_device_ids()) {
        ...
        std::thread thread0([&]() {
            ...
            for (uint32_t loop = 0; loop < num_loops; loop++) {
                uint32_t const response = arc_messenger->send_message((uint32_t)blackhole::ArcMessageType::TEST);
                ASSERT_EQ(response, 0);
            }
        });
