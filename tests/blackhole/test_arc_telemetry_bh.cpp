...
TEST(BlackholeTelemetry, BasicBlackholeTelemetry) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        uint32_t board_id_high = blackhole_arc_telemetry_reader->read_entry(blackhole::TelemetryTag::BOARD_ID_HIGH);
        uint32_t board_id_low = blackhole_arc_telemetry_reader->read_entry(blackhole::TelemetryTag::BOARD_ID_LOW);
...
TEST(BlackholeTelemetry, BlackholeTelemetryEntryAvailable) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        EXPECT_TRUE(blackhole_arc_telemetry_reader->is_entry_available(blackhole::TelemetryTag::BOARD_ID_HIGH));
        EXPECT_TRUE(blackhole_arc_telemetry_reader->is_entry_available(blackhole::TelemetryTag::BOARD_ID_LOW));
