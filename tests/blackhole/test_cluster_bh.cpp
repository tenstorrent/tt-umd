...
//                 std::vector<tt_xy_pair> chan = cluster.get_soc_descriptor(i).dram_cores.at(ch);
...
//                     test_utils::read_data_from_device(
//                         cluster, readback_vec, tt_cxy_pair(0, core), address, 40, "SMALL_READ_WRITE_TLB");
//                     ASSERT_EQ(vector_to_write, readback_vec) << "Vector read back from core " << core.x << "-" <<
//                     core.y
...
//         (eth_version >= tt_version(6, 8, 0) || eth_version == tt_version(6, 7, 241)) &&
//         cluster.translation_tables_en;
