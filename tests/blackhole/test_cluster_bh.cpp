...
//             auto& sdesc = cluster.get_virtual_soc_descriptors().at(i);
...
//             for (auto& core : cluster.get_virtual_soc_descriptors().at(i).workers) {
    ...
    for (int i = 0; i < target_devices.size(); i++) {
        for (int loop = 0; loop < 100;
            ...
            for (int ch = 0; ch < NUM_CHANNELS; ch++) {
                std::vector<tt_xy_pair> chan = cluster.get_virtual_soc_descriptors().at(i).dram_cores.at(ch);
                ...
                cluster.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were commited
                test_utils::read_data_from_device(
                    cluster, readback_vec, tt_cxy_pair(i, subchan), address, 40, "SMALL_READ_WRITE_TLB");
...
TEST(SiliconDriverBH, MultiThreadedDevice) {
    ...
    std::thread th1 = std::thread([&] {
        ...
        for (int loop = 0; loop < 100; loop++) {
            for (auto& core : cluster.get_virtual_soc_descriptors().at(0).workers) {
                cluster.write_to_device(
                    vector_to_write.data(),
                    vector_to_write.size() * sizeof(std::uint32_t),
                    tt_cxy_pair(0, core),
    ...
    std::thread th2 = std::thread([&] {
        ...
        for (auto& core_ls : cluster.get_virtual_soc_descriptors().at(0).dram_cores) {
            for (int loop = 0; loop < 100; loop++) {
...
TEST(SiliconDriverBH, MultiThreadedMemBar) {
    ...
    for (int chan = 0; chan < cluster.get_virtual_soc_descriptors().at(0).get_num_dram_channels(); chan++) {
        auto core = cluster.get_virtual_soc_descriptors().at(0).get_core_for_dram_channel(chan, 0);
        test_utils::read_data_from_device(
    ...
    for (auto& core : cluster.get_virtual_soc_descriptors().at(0).ethernet_cores) {
        test_utils::read_data_from_device(
            cluster,
            readback_membar_vec,
            tt_cxy_pair(0, core),
...
TEST(SiliconDriverBH, DISABLED_BroadcastWrite) {  // Cannot broadcast to tensix/ethernet and DRAM simultaneously on
    ...
    for (const auto& size : broadcast_sizes) {
        ...
        for (const auto i : target_devices) {
            for (const auto& core : cluster.get_virtual_soc_descriptors().at(i).workers) {
            ...
            for (int chan = 0; chan < cluster.get_virtual_soc_descriptors().at(i).get_num_dram_channels(); chan++) {
                const auto& core = cluster.get_virtual_soc_descriptors().at(i).get_core_for_dram_channel(chan, 0);
                test_utils::read_data_from_device(
                    cluster, readback_vec, tt_cxy_pair(i, core), address, vector_to_write.size() * 4, "LARGE_READ_TLB");
                ASSERT_EQ(vector_to_write, readback_vec)
