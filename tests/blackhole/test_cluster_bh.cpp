...
TEST(ClusterBH, CreateCluster) {
    std::unique_ptr<Cluster> cluster = std::make_unique<Cluster>();

    for (chip_id_t chip : cluster->get_target_device_ids()) {
        std::cout << "chip " << chip << std::endl;

        std::cout << "ETH harvesting mask " << cluster->get_soc_descriptor(chip).harvesting_masks.eth_harvesting_mask
                  << std::endl;

        std::vector<CoreCoord> dram_cores = cluster->get_soc_descriptor(chip).get_cores(CoreType::ETH, CoordSystem::LOGICAL);
        for (const CoreCoord& core : dram_cores) {
            std::cout << "ETH core " << core.x << "-" << core.y << std::endl;

            const CoreCoord translated_dram_core =
                cluster->get_soc_descriptor(chip).translate_coord_to(core, CoordSystem::TRANSLATED);

            std::cout << "Translated ETH core " << translated_dram_core.x << "-" << translated_dram_core.y
                      << std::endl;
        }

        std::vector<CoreCoord> harvested_cores = cluster->get_soc_descriptor(chip).get_harvested_cores(CoreType::ETH);

        for (const CoreCoord& core : harvested_cores) {
            std::cout << "Harvested core " << core.x << "-" << core.y << std::endl;
        
            const CoreCoord translated_harvested_core =
                cluster->get_soc_descriptor(chip).translate_coord_to(core, CoordSystem::TRANSLATED);

            std::cout << "Translated harvested core " << translated_harvested_core.x << "-" << translated_harvested_core.y
                      << std::endl;
        }
    }
}
...
// //                 cluster.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were
// commited
...
// //                     << "Vector read back from core " << core.x << "-" << core.y << "does not match what was
// written";
...
//                 cluster.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were commited
//                 test_utils::read_data_from_device(cluster, readback_vec, i, core, address, 40,
//                 "SMALL_READ_WRITE_TLB"); ASSERT_EQ(vector_to_write, readback_vec)
//                     << "Vector read back from core " << core.str() << " does not match what was written";
//                 cluster.wait_for_non_mmio_flush();
//                 cluster.write_to_device(
...
//         ASSERT_EQ(
//             readback_membar_vec.at(0), 187);  // Ensure that memory barriers were correctly initialized on all
//             workers
//         readback_membar_vec = {};
//     }
...
//                                                   // Blackhole .. wait_for_non_mmio_flush() is not working as
//                                                   expected?
...
//                 }
//                 test_utils::read_data_from_device(
//                     cluster, readback_vec, tt_cxy_pair(i, core), address, vector_to_write.size() * 4,
//                     "LARGE_READ_TLB");
//                 ASSERT_EQ(vector_to_write, readback_vec)
...
//                 ASSERT_EQ(vector_to_write, readback_vec) << "Vector read back from DRAM core " << i << " " <<
//                 core.str()
