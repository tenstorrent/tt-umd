...
void move_data(
    ...
    test_utils::read_data_from_device(
        ...
        readback_vec,
        sender_core.chip,
        device.get_soc_descriptor(sender_core.chip).get_coord_at(sender_core.core, sender_core.core.coord_type),
        sender_core.addr,
        size);
    device.write_to_device(
        ...
        receiver_core.chip,
        device.get_soc_descriptor(receiver_core.chip).get_coord_at(receiver_core.core, receiver_core.core.coord_type),
        receiver_core.addr);
    ...
    for (const auto& receiver_core : receiver_cores) {
        device.write_to_device(
            ...
            device.get_soc_descriptor(receiver_core.chip)
                .get_coord_at(receiver_core.core, receiver_core.core.coord_type),
            receiver_core.addr);
    }
    device.wait_for_non_mmio_flush();  // Barrier to ensure that all writes over ethernet were commited
