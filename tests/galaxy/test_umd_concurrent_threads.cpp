...
TEST(GalaxyConcurrentThreads, WriteToAllChipsL1) {
    ...
    tt_SiliconDevice device = tt_SiliconDevice(
        tt::ARCH::WORMHOLE_B0, cluster_desc_path, all_devices, num_host_mem_ch_per_mmio_device, dynamic_tlb_config, false, true);
...
TEST(GalaxyConcurrentThreads, PushInputsWhileSignalingCluster) {
    ...
    tt_SiliconDevice device = tt_SiliconDevice(
        tt::ARCH::WORMHOLE_B0, cluster_desc_path, target_devices, num_host_mem_ch_per_mmio_device, dynamic_tlb_config, false, true);
    const auto sdesc_per_chip = device.get_virtual_soc_descriptors();
