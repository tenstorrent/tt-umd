...
TEST(GalaxyConcurrentThreads, WriteToAllChipsL1) {
    ...
    std::thread th1 = std::thread([&] {
        ...
        for (const auto& chip : target_devices_th1) {
            for (auto& core : sdesc_per_chip.at(chip).workers) {
                device.write_to_device(vector_to_write_th1, cxy_pair(chip, core), address, "SMALL_READ_WRITE_TLB");
            }
        }
        ...
        for (auto& chip : target_devices_th1) {
            for (auto& core : sdesc_per_chip.at(chip).workers) {
                device.read_from_device(
                    readback_vec, cxy_pair(chip, core), address, write_size, "SMALL_READ_WRITE_TLB");
    ...
    std::thread th2 = std::thread([&] {
        ...
        for (const auto& chip : target_devices_th2) {
            for (auto& core : sdesc_per_chip.at(chip).workers) {
                device.write_to_device(vector_to_write_th2, cxy_pair(chip, core), address, "SMALL_READ_WRITE_TLB");
            }
        }
        device.wait_for_non_mmio_flush();
        for (const auto& chip : target_devices_th2) {
    ...
    std::thread th1 = std::thread([&] {
        ...
        for (const auto& chip : target_devices_th1) {
            for (auto& core : dram_cores) {
                device.write_to_device(vector_to_write, cxy_pair(chip, core), address, "SMALL_READ_WRITE_TLB");
            }
        }
        device.wait_for_non_mmio_flush();
        for (const auto& chip : target_devices_th1) {
...
TEST(GalaxyConcurrentThreads, PushInputsWhileSignalingCluster) {
    ...
    std::thread th1 = std::thread([&] {
        ...
        device.write_to_device(large_vector, cxy_pair(mmio_chip, xy_pair(0, 0)), address, "SMALL_READ_WRITE_TLB");
        device.read_from_device(
            readback_vec,
            cxy_pair(mmio_chip, xy_pair(0, 0)),
            address,
            large_vector.size() * 4,
    ...
    std::thread th2 = std::thread([&] {
        ...
        for (const auto& chip : target_devices) {
            for (auto& core : sdesc_per_chip.at(chip).workers) {
                device.write_to_device(small_vector, cxy_pair(chip, core), address, "SMALL_READ_WRITE_TLB");
            }
        }
        device.wait_for_non_mmio_flush();
        for (const auto& chip : target_devices) {
            for (auto& core : sdesc_per_chip.at(chip).workers) {
                device.read_from_device(
                    readback_vec, cxy_pair(chip, core), address, small_vector.size() * 4, "SMALL_READ_WRITE_TLB");
