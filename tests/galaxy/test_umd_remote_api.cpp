...
void run_remote_read_write_test(uint32_t vector_size, bool dram_write) {
    ...
    float const write_size = vector_to_write.size() * 4;
...
TEST(GalaxyDataMovement, TwoChipMoveData1) {
    SocDescriptor const sdesc(tt::ARCH::WORMHOLE_B0, {.noc_translation_enabled = true});
...
TEST(GalaxyDataMovement, BroadcastData1) {
    ...
    tt_multichip_core_addr const sender_core(4, CoreCoord(18, 18, CoreType::TENSIX, CoordSystem::TRANSLATED), 0x5000);
...
TEST(GalaxyDataMovement, BroadcastData2) {
    ...
    tt_multichip_core_addr const sender_core(12, CoreCoord(18, 18, CoreType::TENSIX, CoordSystem::TRANSLATED), 0x5000);
    std::vector<tt_multichip_core_addr> receiver_cores;
...
TEST(GalaxyDataMovement, BroadcastData3) {
    ...
    tt_multichip_core_addr const sender_core(10, CoreCoord(0, 0, CoreType::DRAM, CoordSystem::TRANSLATED), 0x20000);
    std::vector<tt_multichip_core_addr> receiver_cores;
...
TEST(GalaxyDataMovement, BroadcastData4) {
    ...
    tt_multichip_core_addr const sender_core(17, CoreCoord(24, 24, CoreType::TENSIX, CoordSystem::TRANSLATED), 0x20000);
    std::vector<tt_multichip_core_addr> receiver_cores;
...
TEST(GalaxyDataMovement, BroadcastData5) {
    ...
    tt_multichip_core_addr const sender_core(31, CoreCoord(19, 19, CoreType::TENSIX, CoordSystem::TRANSLATED), 0x20000);
    std::vector<tt_multichip_core_addr> receiver_cores;
...
TEST(GalaxyDataMovement, DISABLED_BroadcastData6) {
    SocDescriptor const sdesc(tt::ARCH::WORMHOLE_B0, {.noc_translation_enabled = true});
    tt_multichip_core_addr const sender_core(1, CoreCoord(18, 18, CoreType::TENSIX, CoordSystem::TRANSLATED), 0x5000);
    std::vector<tt_multichip_core_addr> receiver_cores;
    for (int i = 2; i < 33; ++i) {
        receiver_cores.push_back(
