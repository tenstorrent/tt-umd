...
TEST(SiliconDriverGS, CreateDestroySequential) {
    ...
    for(int i = 0; i < 100; i++) {
        tt_SiliconDevice device = tt_SiliconDevice(num_host_mem_ch_per_mmio_device, false, true);
...
TEST(SiliconDriverGS, CreateMultipleInstance) {
    ...
    for(int i = 0; i < 100; i++) {
        concurrent_devices.insert({i, new tt_SiliconDevice(num_host_mem_ch_per_mmio_device, false, true)});
