...
// Over budget with no veto callback: the action fires.
...
// Over budget but the veto reports the op healthy: the action is suppressed.
TEST(OpTimeoutGuard, HealthyVetoSuppressesAction) {
    bool fired = false;
    auto guard = OpTimeoutGuard(
        0ms, [] { return false; }, [&](std::chrono::nanoseconds, int) { fired = true; });
    guard.record_and_check(std::chrono::steady_clock::now(), 42);
    EXPECT_FALSE(fired);
}
...
TEST(OpTimeoutGuard, ActionThrowsArbitraryException) {
    auto guard = OpTimeoutGuard(
        0ms, std::function<bool()>{}, [](std::chrono::nanoseconds, int) { throw std::runtime_error("over budget"); });
    EXPECT_THROW(guard.record_and_check(std::chrono::steady_clock::now(), 42), std::runtime_error);
}
