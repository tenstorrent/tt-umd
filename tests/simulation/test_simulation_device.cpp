...
TEST_P(LoopbackAllCoresParam, LoopbackSingleTensix) {
    ...
    CoreCoord core = soc_desc.get_coord_at(GetParam(), CoordSystem::NOC0);
...
TEST_P(LoopbackAllCoresParam, LoopbackStressSize) {
    auto &soc_desc = device->get_soc_descriptor();
    CoreCoord core = soc_desc.get_coord_at(GetParam(), CoordSystem::NOC0);
    CoreCoord dram = soc_desc.get_coord_at({1, 0}, CoordSystem::NOC0);
    if (core == dram) {
...
TEST_F(SimulationDeviceFixture, LoopbackTwoTensix) {
    ...
    CoreCoord core1 = soc_desc.get_coord_at({0, 1}, CoordSystem::NOC0);
    CoreCoord core2 = soc_desc.get_coord_at({1, 1}, CoordSystem::NOC0);
