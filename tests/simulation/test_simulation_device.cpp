...
std::vector<uint32_t> generate_data(uint32_t size_in_bytes) {
    size_t const size = size_in_bytes / sizeof(uint32_t);
...
TEST_P(LoopbackAllCoresParam, LoopbackSingleTensix) {
    ...
    CoreCoord const core = soc_desc.get_coord_at(GetParam(), CoordSystem::TRANSLATED);
...
TEST_P(LoopbackAllCoresParam, LoopbackStressSize) {
    auto &soc_desc = device->get_soc_descriptor();
    CoreCoord const core = soc_desc.get_coord_at(GetParam(), CoordSystem::TRANSLATED);
    CoreCoord const dram = soc_desc.get_coord_at({1, 0}, CoordSystem::TRANSLATED);
    if (core == dram) {
...
TEST_F(SimulationDeviceFixture, LoopbackTwoTensix) {
    ...
    CoreCoord const core1 = soc_desc.get_coord_at({0, 1}, CoordSystem::TRANSLATED);
    CoreCoord const core2 = soc_desc.get_coord_at({1, 1}, CoordSystem::TRANSLATED);
...
TEST_F(SimulationDeviceFixture, SimpleApiTest) {
    ...
    RiscType const example_dm_cores = RiscType::DM0 | RiscType::DM1 | RiscType::DM7;
