...
TEST(SiliconDriverWH, MultiThreadedMemBar) {
    ...
    for (int chan = 0; chan < cluster.get_soc_descriptor(0).get_num_dram_channels(); chan++) {
        ...
        test_utils::read_data_from_device(cluster, readback_membar_vec, tt_cxy_pair(0, core), 0, 4);
        ASSERT_EQ(
...
TEST(SiliconDriverWH, SysmemTestWithPcie) {
    ...
    cluster.read_from_device_reg(&buffer[0], PCIE_CORE, base_address, buffer.size());
    ...
    cluster.write_to_device_reg(&buffer[0], buffer.size(), PCIE_CORE, base_address);
    ...
    cluster.read_from_device_reg(&throwaway[0], PCIE_CORE, base_address, throwaway.size());
...
TEST(SiliconDriverWH, LargeAddressTlb) {
    ...
    cluster.read_from_device(&value1, ARC_CORE_CHIP, addr, sizeof(uint32_t));

    // Read the scratch register via a different TLB, different code path:
    cluster.read_from_device_reg(&value2, ARC_CORE_CHIP, addr, sizeof(uint32_t));
