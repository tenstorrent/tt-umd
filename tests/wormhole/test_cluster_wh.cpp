...
TEST(SiliconDriverWH, DMA2) {
    ...
    std::vector<uint8_t> pattern(buf_size);
    tt_cxy_pair local_core(mmio_chip_id, dram.x, dram.y);
    tt_cxy_pair remote_core(remote_chip_id, dram.x, dram.y);
    test_utils::fill_with_random_bytes(&pattern[0], pattern.size());

    cluster.set_dma_enabled(true);
    cluster.write_to_device(&pattern[0], pattern.size(), local_core, 0x0, "LARGE_WRITE_TLB");
    cluster.write_to_device(&pattern[0], pattern.size(), remote_core, 0x0, "LARGE_WRITE_TLB");

    std::vector<uint8_t> readback_local(pattern.size());
    cluster.read_from_device(&readback_local[0], local_core, 0x0, pattern.size(), "LARGE_READ_TLB");
    ASSERT_EQ(pattern, readback_local);

    std::vector<uint8_t> readback_remote(pattern.size());
    cluster.read_from_device(&readback_remote[0], remote_core, 0x0, pattern.size(), "LARGE_READ_TLB");
    ASSERT_EQ(pattern, readback_remote);
}