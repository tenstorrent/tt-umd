...
TEST(RemoteCommunicationWormhole, LargeTransferNoSysmem) {
    ...
    auto remote_communication = RemoteCommunication::create_remote_communication(local_tt_device.get(), target_chip, nullptr);  // nullptr for sysmem_manager
    ...
    remote_tt_device->write_to_device(
        data_to_write.data(), tensix_core_xy, test_address, test_size);
    remote_tt_device->wait_for_non_mmio_flush();
    remote_tt_device->read_from_device(
        data_read.data(), tensix_core_xy, test_address, test_size);
    
    // Verify data matches
    ASSERT_EQ(data_to_write.size(), data_read.size()) 
        << "Read and write data sizes do not match";
    for (size_t i = 0; i < data_to_write.size(); i++) {
        ASSERT_EQ(data_to_write[i], data_read[i])
            << "Data mismatch at index " << i << ": expected 0x" 
            << std::hex << data_to_write[i] << " but got 0x" << data_read[i];
    }
