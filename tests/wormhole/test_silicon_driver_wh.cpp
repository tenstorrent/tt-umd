...
TEST(SiliconDriverWH, StaticTLB_RW) {
    ...
    for(int i = 0; i < vector_to_write.size(); i++) vector_to_write.at(i) = i + 16;
    ...
    device.broadcast_write_to_cluster(vector_to_write.data(), vector_to_write.size(), address, {}, rows_to_exclude, cols_to_exclude);
