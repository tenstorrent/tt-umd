...
int main(int argc, char* argv[]) {
    ...
    auto print_core_all_systems = [&](const SocDescriptor& soc_desc, const CoreCoord& core) {
        for (CoordSystem const coord_system :
    ...
    auto print_cores = [&](ChipId chip, CoreType core_type) {
        std::string const core_type_str = to_str(core_type);
    ...
    for (ChipId const chip : cluster->get_target_device_ids()) {
        ...
        HarvestingMasks const harvesting_masks = cluster->get_cluster_description()->get_harvesting_masks(chip);
