...
**Problem:** Functions return negative `tt_result_t` values for
errors (e.g., `return -(tt_errno = TT_ENODEV)`), but `tt_result_t`
is an enum. In C, enum signedness is implementation-defined. Since
all enum values are positive (0-64), compilers may treat it as
unsigned, breaking negative return semantics.

**Current pattern:**
```c
tt_result_t tt_device_open(tt_device_t *dev) {
    return -(tt_errno = TT_ENODEV);  // Returns -10
}
```

**Options:**
- **A)** Change return type to `int` (POSIX-style, explicitly
  signed)
- **B)** Return `tt_result_t` directly without negation, check
  `!= TT_OK`
- **C)** Use `ssize_t` where applicable, `int` elsewhere

**Decision:** Deferred. Need to choose consistent error return
convention.
