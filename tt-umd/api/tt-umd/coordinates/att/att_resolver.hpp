...
#include "tt-umd/coordinates/att/att_map.hpp"
 ...
 * needs to reach it.
 *
 * Wormhole and Blackhole carry the destination core out of band, leaving the address on the wire
 * core-local. Quasar is the opposite: the ATT resolves a flat address into a destination core plus
 * a local address, so the driver folds the coordinate into the address.
 */
class EndpointResolver {
public:
    /** @param map Window and selector tables for the target product. */
    explicit EndpointResolver(const MapData& map);
...
private:
     ...
     * Indexed by WindowClass, like map_.windows and map_.endpoint_words, so one index names a
     * window, the rows it resolves through, and the inverse of those rows.
     */
