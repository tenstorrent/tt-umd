...
CoreCoord SocDescriptor::translate_coord_to(const CoreCoord core_coord, const CoordSystem coord_system) const {
    CoreCoord new_core_coord = core_coord;
    new_core_coord.coord_system = coord_system;
    return new_core_coord;
}

CoreCoord SocDescriptor::get_coord_at(const tt_xy_pair core, const CoordSystem coord_system) const {
    return CoreCoord(core, CoreType::TENSIX, coord_system);
}
...
CoreCoord SocDescriptor::translate_coord_to(
    ...
    return translate_coord_to(get_coord_at(core_location, input_coord_system), target_coord_system);
}
...
inline constexpr tt_xy_pair GRID_SIZE = {2, 4};
const static std::vector<tt_xy_pair> TENSIX_CORES_NOC0 = {{0, 0}, {0, 1}, {1, 0}, {1, 1}};

// Dispatch Cores?
const static std::vector<tt_xy_pair> DISPATCH_CORES_NOC0 = {{0, 2}, {1, 2}};

// Are DRAM same as PCI?
static const std::vector<std::vector<tt_xy_pair>> DRAM_CORES_NOC0 = {{{0, 3}, {1, 3}}};

const static std::vector<tt_xy_pair> PCIE_CORES_NOC0 = {{0, 3}, {1, 3}};

SocDescriptorInfo SocDescriptor::get_soc_descriptor_info(tt::ARCH arch) {
    return SocDescriptorInfo{
        .arch = tt::ARCH::QUASAR,
        .grid_size = GRID_SIZE,
        .tensix_cores = TENSIX_CORES_NOC0,
        .dram_cores = DRAM_CORES_NOC0,
        .eth_cores = {},
        .arc_cores = {},
        .pcie_cores = PCIE_CORES_NOC0,
        .router_cores = {},
        .security_cores = {},
        .l2cpu_cores = {},
...
void SocDescriptor::get_cores_and_grid_size_from_coordinate_manager() {
    auto build_cores = [](const std::vector<tt_xy_pair> &xy_pairs, CoreType type) {
        std::vector<CoreCoord> cores;
        cores.reserve(xy_pairs.size());
        for (const auto &p : xy_pairs) {
            cores.emplace_back(p.x, p.y, type, CoordSystem::NOC0);
        }
        return cores;
    };

    cores_map[CoreType::TENSIX] = build_cores(TENSIX_CORES_NOC0, CoreType::TENSIX);
    grid_size_map[CoreType::TENSIX] = calculate_grid_size(TENSIX_CORES_NOC0);

    cores_map[CoreType::PCIE] = build_cores(PCIE_CORES_NOC0, CoreType::PCIE);
    grid_size_map[CoreType::PCIE] = calculate_grid_size(PCIE_CORES_NOC0);

    // DRAM is nested per channel; flatten for cores_map and keep the nested form in dram_cores_core_coord.
    dram_cores_core_coord.clear();
    std::vector<CoreCoord> all_dram;
    for (const auto &channel : DRAM_CORES_NOC0) {
        std::vector<CoreCoord> channel_cores;
        channel_cores.reserve(channel.size());
        for (const auto &p : channel) {
            CoreCoord cc(p.x, p.y, CoreType::DRAM, CoordSystem::NOC0);
            channel_cores.push_back(cc);
            all_dram.push_back(cc);
        }
        dram_cores_core_coord.push_back(std::move(channel_cores));
    }
    cores_map[CoreType::DRAM] = std::move(all_dram);
    grid_size_map[CoreType::DRAM] = {
        DRAM_CORES_NOC0.size(), DRAM_CORES_NOC0.empty() ? size_t{0} : DRAM_CORES_NOC0[0].size()};

    // Empty entries for the remaining types so lookups don't throw.
    for (auto t : {CoreType::ETH, CoreType::ARC, CoreType::ROUTER_ONLY, CoreType::SECURITY, CoreType::L2CPU}) {
        cores_map[t] = {};
        grid_size_map[t] = {0, 0};
        harvested_cores_map[t] = {};
        harvested_grid_size_map[t] = {0, 0};
    }
    for (auto t : {CoreType::TENSIX, CoreType::DRAM, CoreType::PCIE}) {
        harvested_cores_map[t] = {};
        harvested_grid_size_map[t] = {0, 0};
    }
}
...
uint32_t SocDescriptor::get_num_eth_channels() const { return 0; }

uint32_t SocDescriptor::get_num_harvested_eth_channels() const { return 0; }
