...
def get_pcie_gen(speed_str):
    ...
    if speed_str == "N/A":
        return 0
    # Handle sysfs output
    if not speed_str or "GT/s" not in speed_str:
        return 0
    try:
    ...
    try:
        for dev_name in os.listdir(dev_path):
            try:
                ...
                print(
                    f"INFO: Could not stat {os.path.join(dev_path, dev_name)}: {e}",
                    file=sys.stderr,
                )
        ...
        for device_dir in os.listdir(class_path):
            ...
            try:
                ...
                if minor_num in usable_minor_nums:
                    ...
                    pci_device_path = os.path.realpath(
                        os.path.join(dev_class_entry, "device")
                    )
...
def export_data(data, base_filename, do_json=False, do_csv=False, do_md=False):
    ...
    if do_csv or do_md:
        ...
        if pcie_list:
            ...
            summary = ", ".join(
                [
                    f"x{c.get('pcie_width', '?')} Gen{c.get('pcie_speed', '?')}"
                    for c in pcie_list
                ]
            )
    ...
    if do_md:
        try:
            md_table = df.T.reset_index().to_markdown(
                index=False, headers=["Metric", "Value"], tablefmt="github"
            )
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Output to all formats (JSON, CSV, MD)."
    )
    parser.add_argument(
        "--no-file",
        action="store_true",
    ...
    if args.no_file and base_filename is not None:
        print(
            "Error: --no-file cannot be used together with --output/-o.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if args.no_file:
        if len(selected_formats) > 1:
            print(
                "Error: With --no-file, specify only one of --json, --csv, or --md.",
                file=sys.stderr,
            )
            sys.exit(2)
