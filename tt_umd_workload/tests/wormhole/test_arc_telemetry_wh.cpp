...
#include "tt_umd/arc/smbus_arc_telemetry_reader.hpp"
...
TEST(WormholeTelemetry, BasicWormholeTelemetry) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        uint32_t board_id_low = blackhole_arc_telemetry_reader->read_entry(wormhole::LegacyTelemetryTag::BOARD_ID_LOW);
...
TEST(TestTelemetry, CompareTwoTelemetryValues) {
    ...
    for (int pci_device_id : pci_device_ids) {
        ...
        EXPECT_EQ(
            ...
            smbus_telemetry_reader->read_entry(wormhole::LegacyTelemetryTag::DM_BL_FW_VERSION));

        EXPECT_EQ(
            arc_telemetry_reader->read_entry(TelemetryTag::DM_APP_FW_VERSION),
            smbus_telemetry_reader->read_entry(wormhole::LegacyTelemetryTag::DM_APP_FW_VERSION));

        EXPECT_EQ(
            arc_telemetry_reader->read_entry(TelemetryTag::TT_FLASH_VERSION),
            smbus_telemetry_reader->read_entry(wormhole::LegacyTelemetryTag::TT_FLASH_VERSION));
